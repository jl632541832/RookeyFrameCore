...
namespace Rookey.Frame.UIOperate
{
    ...
    public abstract class UIFrameFactory
    {
        ...
        /// <returns></returns>
        public static UIFrameFactory GetInstance(HttpRequest request)
        {
        ...
        /// <param name="request">请求对象</param>
        /// <returns></returns>
        public abstract string GetEditFormHTML(Guid moduleId, Guid? id, string gridId = null, Guid? copyId = null, bool showTip = false, Guid? todoTaskId = null, Guid? formId = null, HttpRequest request = null);
        ...
        /// <param name="request">请求对象</param>
        /// <returns></returns>
        public abstract string GetViewFormHTML(Guid moduleId, Guid id, string gridId = null, string fromEditPageFlag = null, bool isRecycle = false, bool showTip = false, Guid? formId = null, HttpRequest request = null);
        ...
        /// <param name="viewId">视图ID</param>
        /// <returns></returns>
        public abstract string GetExportModelHTML(Guid moduleId, int cc, Guid? viewId = null);
        ...
        /// <param name="request">请求对象</param>
        /// <returns></returns>
        public abstract string GetDialogTreeHTML(Guid moduleId, HttpRequest request);
        ...
        /// <returns></returns>
        public abstract string GetDesktopGridHTML(Guid moduleId, int top = 5, string sortName = "Id", bool isDesc = true, Dictionary<string, object> dic = null, HttpRequest request = null);
        ...
        internal static int GetEditMode(Sys_Module module, Sys_Form form, out int formWidth, out int formHeight, Guid? userId = null)
        {
            ...
            if (form.ModuleEditModeOfEnum == ModuleEditModeEnum.TabFormDetailExpand)
                return (int)ModuleEditModeEnum.TabFormEdit;
        ...
        internal static string GetFormFieldInputValue(out string textValue, Sys_Module module, Sys_FormField field, Sys_Field sysField, object model, object copyModel = null, bool canOp = true, bool isRestartFlow = false, UserInfo currUser = null)
        {
            ...
            if (model == null) //新增页面
            {
                ...
                else //未启用编码规则
                {
                    ...
                    if (field.IsAllowAdd != false && copyModel != null) //字段允许新增并且复制对象不为空
                    {
                ...
                else if (SystemOperate.IsDictionaryBindField(module.Id, sysField.Name))
                {
                    ...
                    if (dics.Count > 0) //有字典值时
                    {
                        if (model == null) //新增时
                        {
                            ...
                            {
                                if (field.IsMultiSelect == true) //多选
                                {
                                    List<string> vs = value.ObjToStr().Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries).ToList();
                                    textValue = string.Join(",", dics.Where(x => vs.Contains(x.Value)).Select(x => x.Name));
                                }
                                else
                                {
                    ...
                    field.ControlTypeOfEnum == ControlTypeEnum.DateTimeBox)
            {
                if (value.ObjToStr() != string.Empty)
                {
                    ...
                    try
                    {
                        value = DateTime.Parse(value.ObjToStr()).ToString(dateFormat);
                    }
                    catch
                    {
            ...
            {
                if (field != null && field.DefaultValue == "currDate")
                {
                    try
                    {
                        textValue = ((DateTime)value).ToString("yyyy-MM-dd");
                    }
                    catch { }
                }
            }
            ...
            {
                if (!string.IsNullOrEmpty(sysField.ForeignModuleName)) //外键模块
                {
                    bool isMutiSelect = field.IsMultiSelect == true && fieldType == typeof(String);
                    ...
                    if (foreignModule != null)
                    {
                        ...
                        else if (string.IsNullOrEmpty(textValue)) //单选外键
                        {
                            textValue = CommonOperate.GetModelTitleKeyValue(foreignModule.Id, value.ObjToGuid());
                        }
                    }
                }
            }
        ...
        internal static string GetFormFieldViewValue(Sys_Module module, Sys_Form form, Sys_FormField field, Sys_Field sysField, object model, Guid id, bool canView, bool canEdit, bool hasRecordEditPower)
        {
            ...
            if (canView) //有字段查看权限
            {
                ...
                object value = model == null ? null : CommonOperate.GetModelFieldValueByModel(module.Id, model, sysField);
            ...
            #endregion
            string valueSpan = string.Format("<span id=\"span_{0}\" class=\" dspan \" value=\"{2}\">{1}</span>", sysField.Name, valueStr, v);
        ...
        {
            if (sysField == null || field == null)
                return string.Empty;
            #region 变量定义
            StringBuilder sb = new StringBuilder();
            bool isDisabled = false; //不允许编辑控件
            bool foreignFieldBindMid = false;
            if (sysField.Sys_ModuleId.HasValue && sysField.Sys_ModuleId.Value != Guid.Empty)
            {
                foreignFieldBindMid = moduleId != sysField.Sys_ModuleId.Value;
            ...
            if (isAllowOp)
            {
                ...
                if (isSearchForm && controlTypeEnum == ControlTypeEnum.LabelBox) //搜索表单并且是Label控件
                {
                    ...
                    else if (SystemOperate.IsForeignField(sysField))
                    {
                        controlTypeEnum = ControlTypeEnum.DialogGrid;
                    }
                    ...
                    {
                        controlTypeEnum = ControlTypeEnum.DateBox;
                    }
                    else
                    {
                    ...
                    case ControlTypeEnum.TextBox: //文本框
                        {
                            ...
                            string textareaClass = string.Empty;
                            if (!isSearchForm) //非搜索表单
                            {
                                if (controlTypeEnum == ControlTypeEnum.TextAreaBox)
                                {
                                    inputOptions += ",multiline:true";
                                    textareaClass = "textarea-flag";
                                }
                            ...
                            inputOptions += "\"";
                            sb.AppendFormat("<input id=\"{0}\" name=\"{0}\" noforein=\"1\" class=\"easyui-textbox {5}\" value=\"{1}\" style=\"width:{2};\" {3} {4}/>",
                                sysField.Name, value.ObjToStr(), inputWidthStr, inputOptions, linkFieldsStr, textareaClass);
                            if (module.Name == "表单字段" && sysField.Name == "UrlOrData")
                            {
                                //添加一个添加数据源的按钮
                                int w = 0;
                                int h = 0;
                                int editMode = GetEditMode(module, form, out w, out h, currUser.UserId);
                    ...
                    case ControlTypeEnum.DialogTree: //弹出选择树
                        {
                            ...
                            if (foreignModule != null) //当前字段是外键字段
                            {
                                ...
                                {
                                    fieldUrl = controlTypeEnum == ControlTypeEnum.DialogGrid ? string.Format("/Page/Grid.html?page=fdGrid&moduleId={0}&initModule={1}&initField={2}", foreignModuleId.ToString(), HttpUtility.UrlEncode(module.Name), fieldName) :
                                              string.Format("/Page/DialogTree.html?moduleName={0}", HttpUtility.UrlEncode(foreignModuleName));
                                    if (isMutiSelect) fieldUrl += "&ms=1";
                                    if (controlTypeEnum == ControlTypeEnum.DialogTree)
                                    {
                                        if (foreignModule.TableName == "OrgM_Emp")
                                            fieldUrl += "&onlyleaf=1";
                                        if (!isSearchForm)
                                            fieldUrl += "&selectIds=" + value.ObjToStr();
                                    }
                                }
                            ...
                            if (isSearchForm && foreignFieldBindMid)
                            {
                                fieldAttr += string.Format(" moduleId=\"{0}\"", moduleId);
                            }
                            inputOptions += "\"";
                    ...
                    case ControlTypeEnum.DateTimeBox: //日期时间型
                        {
                            ...
                            if (controlTypeEnum == ControlTypeEnum.DateTimeBox)
                    ...
                    case ControlTypeEnum.ComboTree: //下拉树
                        {
                            ...
                            if (controlTypeEnum == ControlTypeEnum.ComboTree)
                            {
                            ...
                            else
                            {
                                ...
                                if (SystemOperate.IsEnumField(moduleId, sysField.Name) ||
                                    ...
                                    (fieldUrl != null && fieldUrl.StartsWith("[{") && fieldUrl.EndsWith("}]")))
                            ...
                            string combAttr = string.Empty;
                            if (field.IsMultiSelect.HasValue && field.IsMultiSelect.Value)
                            {
                                inputOptions += ",multiple:true";
                                string attrFlag = "ismuticombo";
                                if (controlTypeEnum == ControlTypeEnum.ComboTree)
                                    attrFlag = "ismuticombotree";
                                else if (controlTypeEnum == ControlTypeEnum.ComboGrid)
                                    attrFlag = "ismuticombogrid";
                                combAttr = string.Format("{0}=\"1\"", attrFlag);
                            }
                            inputOptions += "\"";
                            sb.AppendFormat("<input id=\"{0}\" name=\"{0}\" style=\"width:{1};\" class=\"{4}\" {5} value=\"{2}\" {3}/>",
                                 sysField.Name, inputWidthStr, value.ObjToStr(), inputOptions, className, combAttr);
                            #endregion
                        }
                        break;
                    case ControlTypeEnum.IconBox: //图标控件
                        {
                            ...
                            else
                            {
                                ...
                                sb.AppendFormat("<div name=\"divImg\" class=\"divImg \" ><img id=\"img_{0}\" style=\"height:{2}px;border:0px none;\" src=\"{1}\" />", sysField.Name, iconUrl, ConstDefine.FORM_CONTROL_HEIGHT.ToString());
        ...
        public static string GetAttachmentListHTML(Sys_Module module, Guid? id, FormTypeEnum formType)
        {
            ...
            if (module.FormAttachDisplayStyleOfEnum == FormAttachDisplayStyleEnum.GridStype) //列表方式显示
            {
                ...
                sb.Append("<div id=\"attachPanel\" class=\"easyui-panel atacch-panel\" title=\"附件信息\" style=\"width:100%;height:200px;padding:5px;\" data-options=\"collapsible:true\">");
                ...
                foreach (Sys_Attachment attchement in attachList)
                {
                    ...
                    if (formType == FormTypeEnum.ViewForm || (formType == FormTypeEnum.EditForm && id.HasValue && id.Value != Guid.Empty)) //查看或编辑页面
                    {
            ...
            else //简单方式
            {
                ...
                string panelOptions = "data-options=\"collapsible:false\"";
                ...
                if (attachList != null && attachList.Count > 0)
                {
                    foreach (Sys_Attachment attach in attachList)
                    {
                        ...
                        if (formType == FormTypeEnum.ViewForm || (formType == FormTypeEnum.EditForm && id.HasValue && id.Value != Guid.Empty))
                        {
        ...
        public static string GetApprovalInfoAndOpinionsHtmlTwo(Guid moduleId, Guid? id, Bpm_WorkToDoList todo, UserInfo currUser, bool isParentTodo = false, string domainPath = null)
        {
            ...
            if (id.HasValue && id.Value != Guid.Empty)
            {
                ...
                sb.Append("<div class=\"mt10 mb10 approvalListmt\">");
