...
namespace ServiceStack.OrmLite.MySql
{
    public class MySqlDialectProvider : OrmLiteDialectProviderBase<MySqlDialectProvider>
    {
        ...
        public override string ToColumnExistsStatement(Type modelType, string fieldName)
        {
            return string.Format("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='{0}' AND TABLE_SCHEMA=DATABASE() AND COLUMN_NAME='{1}';",
                 NamingStrategy.GetTableName(GetModel(modelType).ModelName), fieldName);
        }
    }
}
