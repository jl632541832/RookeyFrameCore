...
namespace ServiceStack.OrmLite
{
    public abstract partial class SqlExpression<T> : ISqlExpression
    {
        ...
        public virtual string ToSelectStatement(List<string> fieldNames = null)
        {
            var selectExp = fieldNames != null && fieldNames.Count > 0 ? GetSelectExpression(fieldNames) : SelectExpression;
            var sql = DialectProvider
                .ToSelectStatement(modelDef, selectExp, BodyExpression, OrderByExpression, Offset, Rows);

            return sql;
        }

        public virtual string ToCountStatement(List<string> fieldNames = null)
        {
        ...
        private string GetSelectExpression(List<string> fieldNames = null)
        {
            ...
            foreach (var field in fieldNames)
            {
                if (sqlColumns.Length > 0)
                    sqlColumns.Append(", ");
                sqlColumns.Append(DialectProvider.GetQuotedColumnName(field));
            }
            return "SELECT " + sqlColumns.ToString();
        }
        ...
        protected virtual object VisitStaticArrayMethodCall(MethodCallExpression m)
        {
            switch (m.Method.Name)
            {
                case "Contains":
                    ...
                    object quotedColName = args[args.Count - 1];
    ...
    {
        string ToSelectStatement(List<string> fieldNames = null);
        string SelectInto<TModel>(List<string> fieldNames = null);
    }
