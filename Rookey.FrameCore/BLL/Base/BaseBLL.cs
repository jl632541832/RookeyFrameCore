...
namespace Rookey.Frame.BLL.Base
{
    ...
    public class BaseBLL<T> : IBaseBLL<T> where T : BaseEntity, new()
    {
        ...
        private void BuildDal(UserInfo userInfo, DatabaseType? dbType)
        {
            ...
            try
            {
                Type customerDalType = GetCustomeDALType(typeof(T));
        ...
        {
            return _dal.GetEntitiesByField(out errorMsg, fieldName, fieldValue, expression, where, permissionFilter, orderFields, isDescs, top, fieldNames, references, connString);
        }
