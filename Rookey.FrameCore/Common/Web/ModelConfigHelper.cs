...
namespace Rookey.Frame.Common
{
    ...
    public static class ModelConfigHelper
    {
                ...
                else if (dbType == DatabaseType.MySql) //mysql数据库
                {
                    ...
                    if (token != null && token.Length > 0)
                    {
                        ...
                        foreach (string str in token)
                        {
                            ...
                            else if (str.ToLower().StartsWith("user"))
                            {
                                string[] tempToken = str.Split("=".ToCharArray());
                                if (tempToken != null && tempToken.Length == 2)
                                {
                            ...
                            else if (str.ToLower().StartsWith("password"))
                            {
                                string[] tempToken = str.Split("=".ToCharArray());
                                if (tempToken != null && tempToken.Length == 2)
                                {
        ...
        /// <param name="connString">连接字符串</param>
        /// <param name="currTempDblinkArgs">本地连接对象</param>
        /// <returns></returns>
        public static string GetCrossTableName(string tableName, DatabaseType dbTypeEnum, string connString, DbLinkArgs currTempDblinkArgs = null)
        {
            //额外数据库连接对象
            DbLinkArgs dblinkArgs = GetDbLinkArgs(connString, dbTypeEnum);
            if (dblinkArgs != null)
            {
                ...
                if (dblinkArgs.DataSource == currDblinkArgs.DataSource || dblinkArgs.DataSource == "." || dblinkArgs.DataSource == "(local)") //服务器相同或本地服务器
                {
                    ...
                    {
                        return string.Format("[{0}].[dbo].[{1}]", dblinkArgs.DbName, tableName);
                    }
                }
                else //跨服务器
                {
                    return string.Format("[{0}].[{1}].[dbo].[{2}]", dblinkArgs.DataSource, dblinkArgs.DbName, tableName);
                }
            }
            return tableName;
        }
