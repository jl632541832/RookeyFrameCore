...
namespace Rookey.Frame.Operate.Base
{
    ...
    public static class PermissionOperate
    {
        ...
        {
            string errMsg = string.Empty;
            List<Sys_UserRole> userRoles = CommonOperate.GetEntities<Sys_UserRole>(out errMsg, x => x.Sys_UserId == userId && !x.IsDeleted && !x.IsDraft, null, false);
            if (userRoles == null) userRoles = new List<Sys_UserRole>();
            List<Sys_Role> list = new List<Sys_Role>();
            List<Sys_Role> roles = userRoles.Where(x => x.Sys_RoleId.HasValue && x.Sys_RoleId.Value != Guid.Empty).Select(x => GetRole(x.Sys_RoleId.Value)).ToList();
        ...
        /// </summary>
        /// <param name="exp">条件表达式</param>
        /// <param name="isReference">是否加载导航属性</param>
        /// <returns></returns>
        public static List<Sys_UserRole> GetAllUserRoles(Expression<Func<Sys_UserRole, bool>> exp = null, bool isReference = false)
        {
            string errMsg = string.Empty;
            ...
            List<Sys_UserRole> list = CommonOperate.GetEntities<Sys_UserRole>(out errMsg, tempExp, null, false, null, null, null, null, isReference);
        ...
        public static string GetUserDataPermissionWhere(Guid userId, Guid moduleId, DataPermissionTypeEnum dataPermissionType)
        {
            ...
            string errMsg = string.Empty;
            UserInfo userInfo = UserOperate.GetUserInfo(userId);
            List<Guid?> roleIds = GetUserRoles(userId).Select(x => (Guid?)x.Id).ToList();
            List<Sys_RoleDataPowerConfig> rolePowerConfigs = CommonOperate.GetEntities<Sys_RoleDataPowerConfig>(out errMsg, x => x.Sys_ModuleId == moduleId && roleIds.Contains(x.Sys_RoleId) && !x.IsDeleted, null, false);
            if (rolePowerConfigs != null && rolePowerConfigs.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(powerSql))
                {
                    ...
                    if (userInfo.EmpId.HasValue && userInfo.EmpId.Value != Guid.Empty)
                    {
                        powerSql = powerSql.Replace("{currEmp}", userInfo.EmpId.Value.ToString().ToUpper());
                        EmpExtendInfo userExtend = UserInfo.GetCurrEmpExtendInfo(userInfo).FirstOrDefault();
                        if (userExtend != null)
                        {
                            ...
                            if (powerSql.Contains("{currChildDepts}"))
                            {
                                if (userExtend.PartimePositionIds != null && userExtend.PartimePositionIds.Count > 0) //加载当前人负责的子部门
                                {
                                    foreach (Guid positionId in userExtend.PartimePositionIds)
                                    {
                                        OrgM_DeptDuty position = OrgMOperate.GetPosition(positionId);
                                ...
                                {
                                    List<OrgM_Dept> depts = OrgMOperate.GetChildDepts(deptId);
                            ...
                            {
                                List<Guid> childUserIds = OrgMOperate.GetEmpChildUserIds(userInfo.EmpId.Value, false);
                            ...
                            {
                                List<Guid> childEmpIds = OrgMOperate.GetEmpChildsEmpIds(userInfo.EmpId.Value, false);
                                childEmpIds.Add(userInfo.EmpId.Value);
