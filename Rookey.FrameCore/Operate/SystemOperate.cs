...
namespace Rookey.Frame.Operate.Base
{
    ...
    public static class SystemOperate
    {
        ...
        /// <returns></returns>
        public static Sys_Module GetModuleByRequest(HttpRequest request)
        {
            Guid moduleId = request.QueryEx("moduleId").ObjToGuid();
            string moduleName = HttpUtility.UrlDecode(request.QueryEx("moduleName").ObjToStr());
            Sys_Module module = moduleId != Guid.Empty ? GetModuleById(moduleId) : GetModuleByName(moduleName);
            if (module == null)
            {
                string tableName = request.QueryEx("tableName").ObjToStr();
        ...
        public static string GetModuleJsFilePath(Sys_Module module, bool isFullPath = false)
        {
            ...
            if (module != null && !string.IsNullOrEmpty(module.TableName))
            {
                ...
                string filePath = Globals.GetWebDir() + jsPath.Replace("/", Path.DirectorySeparatorChar.ToString());
            ...
            if (sysFields.Count > 0)
            {
                ...
                if (attachments != null && attachments.Count > 0)
                {
                    foreach (Sys_Attachment tempObj in attachments)
                    {
                        try
                        {
                            string tempFile = string.Format("{0}{1}", Globals.GetWebDir(), tempObj.FileUrl).Replace("/", Path.DirectorySeparatorChar.ToString());
                            string tempPdfFile = string.Format("{0}{1}", Globals.GetWebDir(), tempObj.PdfUrl).Replace("/", Path.DirectorySeparatorChar.ToString());
                            string tempSwfFile = string.Format("{0}{1}", Globals.GetWebDir(), tempObj.SwfUrl).Replace("/", Path.DirectorySeparatorChar.ToString());
                ...
                try
                {
                    string codeFile = string.Format("{0}Config{2}TempModel{2}{1}.code", Globals.GetWebDir(), t.TableName, Path.DirectorySeparatorChar.ToString());
                    if (System.IO.File.Exists(codeFile))
                        System.IO.File.Delete(codeFile);
                    string dllFile = string.Format("{0}TempModel{2}{1}.dll", Globals.GetBinPath(), t.TableName, Path.DirectorySeparatorChar.ToString());
        ...
        public static string GetFieldDisplayValue(Guid moduleId, object model, Sys_Field sysField, Sys_FormField field)
        {
            ...
            else if (SystemOperate.IsDictionaryBindField(moduleId, sysField.Name)) //字典绑定字段
            {
                ...
                {
                    List<string> vs = valueStr.Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries).ToList();
                    List<string> tempVs = new List<string>();
                    foreach (string v in vs)
                    {
                        tempVs.Add(SystemOperate.GetDictionaryDisplayText(moduleId, sysField.Name, v));
                    }
                    valueStr = string.Join(",", tempVs);
                }
                else
                {
        ...
        public static string GetFieldDisplayValue(Guid moduleId, Guid recordId, string fieldName)
        {
            ...
            return GetFieldDisplayValue(moduleId, model, fieldName);
        }
        ...
        /// <summary>
        /// 获取默认网格的字段（外键字段取Name字段）
        /// </summary>
        /// <param name="sysField"></param>
        /// <returns></returns>
        public static Sys_GridField GetDefaultGridFieldOfForeinName(Sys_Field sysField)
        {
            if (sysField != null && sysField.Sys_ModuleId.HasValue && sysField.Sys_ModuleId.Value != Guid.Empty)
            {
                return GetDefaultGridFields(sysField.Sys_ModuleId.Value, true).Where(x => x.Sys_FieldId == sysField.Id && x.IsVisible).FirstOrDefault();
            }
            return null;
        }
        ...
        /// <param name="reference">是否加载导航</param>
        /// <returns></returns>
        public static List<Sys_FormField> GetUserFormFields(Guid userId, Guid moduleId, bool reference = false)
        {
        ...
        /// </summary>
        /// <param name="moduleId">模块Id</param>
        /// <param name="reference">是否加载导航属性</param>
        /// <returns></returns>
        public static List<Sys_FormField> GetDefaultBatchEditFields(Guid moduleId, bool reference = false)
        {
            List<Sys_FormField> list = GetDefaultFormField(moduleId, reference);
        ...
        public static Sys_Form GetUserFinalFormTwo(UserInfo currUser, Guid moduleId, Bpm_WorkToDoList todo, Guid? formId = null)
        {
            ...
            if (isEnabledFlow) //启用流程后
            {
                ...
                {
                    if (BpmOperate.IsAllowLaunchFlow(moduleId, currUser, null)) //是发起人时
                    {
                        form = BpmOperate.GetLaunchNodeForm(moduleId); //取发起节点表单
                    }
                }
            }
            if (form == null || form.Id == SystemOperate.GetDefaultForm(moduleId).Id)
        ...
        /// <param name="moduleId">模块ID</param>
        /// <param name="userId">用户ID</param>
        /// <param name="isEditForm">是否编辑表单，否则为新增表单</param>
        /// <returns></returns>
        public static List<object> GetFormFeildObjects(Guid moduleId, Guid userId, bool isEditForm = true)
        {
            Sys_Module module = SystemOperate.GetModuleById(moduleId);
            if (module == null) return null;
            List<object> fieldObjects = new List<object>();
            List<Sys_FormField> formFields = SystemOperate.GetDefaultFormField(moduleId, true);
            List<string> opFields = isEditForm ? PermissionOperate.GetUserFieldsPermissions(userId, moduleId, FieldPermissionTypeEnum.EditField) :
                                        PermissionOperate.GetUserFieldsPermissions(userId, moduleId, FieldPermissionTypeEnum.AddField);
            ...
            foreach (Sys_FormField field in formFields)
            {
                ...
                bool canEditOp = (isEditForm ? field.IsAllowEdit != false : field.IsAllowAdd != false) && canOp;
        ...
        /// <param name="isEditForm">是否编辑表单，否则为新增表单</param>
        /// <returns></returns>
        public static string GetFormFeildObjectsJson(Guid moduleId, Guid userId, bool isEditForm = true)
        {
            List<object> fieldObjs = GetFormFeildObjects(moduleId, userId, isEditForm);
            string formFieldJson = HttpUtility.UrlEncode(JsonHelper.Serialize(fieldObjs).Replace("\r\n", string.Empty), Encoding.UTF8).Replace("+", "%20");
        ...
        public static List<Sys_IconManage> GetPageIcons(out long total, IconTypeEnum? iconType, PageInfo pageInfo = null)
        {
            ...
            int type = iconType.HasValue ? (int)iconType.Value : 1;
            string errMsg = string.Empty;
        ...
        public static string DeleteAttachment(List<Sys_Attachment> tempAttachments)
        {
            ...
            if (tempRs)
            {
                foreach (Sys_Attachment tempObj in tempAttachments)
                {
                    try
                    {
                        string tempFile = string.IsNullOrEmpty(tempObj.FileUrl) ? string.Empty : string.Format("{0}{1}", Globals.GetWebDir(), tempObj.FileUrl).Replace("/", Path.DirectorySeparatorChar.ToString());
                        string tempPdfFile = string.IsNullOrEmpty(tempObj.PdfUrl) ? string.Empty : string.Format("{0}{1}", Globals.GetWebDir(), tempObj.PdfUrl).Replace("/", Path.DirectorySeparatorChar.ToString());
                        string tempSwfFile = string.IsNullOrEmpty(tempObj.SwfUrl) ? string.Empty : string.Format("{0}{1}", Globals.GetWebDir(), tempObj.SwfUrl).Replace("/", Path.DirectorySeparatorChar.ToString());
                        if (!string.IsNullOrEmpty(tempFile) && System.IO.File.Exists(tempFile))
                        {
                            System.IO.File.Delete(tempFile); //删除原文件
                        }
        ...
        /// <param name="dbLinkArgs">数据库连接参数</param>
        /// <returns></returns>
        public static List<string> GetDbs(DbLinkArgs dbLinkArgs)
        {
            if (dbLinkArgs == null)
                return new List<string>();
            string errMsg = string.Empty;
            ...
            if (dbLinkArgs.DbType == DatabaseType.MsSqlServer)
            {
                sql = "SELECT NAME FROM master.sys.sysdatabases ORDER BY NAME";
                string connStr = string.Format("Data Source={0};Initial Catalog=master;User ID={1};Password={2};Pooling=true;MAX Pool Size=512;Min Pool Size=50;Connection Lifetime=30", dbLinkArgs.DataSource, dbLinkArgs.UserId, dbLinkArgs.Pwd);
                DataTable dt = CommonOperate.ExecuteQuery(out errMsg, sql, null, connStr, dbLinkArgs.DbType);
                if (dt != null && dt.Rows.Count > 0)
                {
                    return dt.Rows.Cast<DataRow>().Select(x => x[0].ObjToStr()).ToList();
                }
            }
            return new List<string>();
        }
        ...
        /// 获取数据表集合
        /// </summary>
        /// <param name="dbLinkArgs">数据库连接对象</param>
        /// <returns></returns>
        public static List<string> GetDts(DbLinkArgs dbLinkArgs)
        {
            string errMsg = string.Empty;
            string sql = string.Empty;
            if (dbLinkArgs.DbType == DatabaseType.MsSqlServer)
            {
                ...
                DataTable dt = CommonOperate.ExecuteQuery(out errMsg, sql, null, dbLinkArgs.ConnString, dbLinkArgs.DbType);
                if (dt != null && dt.Rows.Count > 0)
                {
                    return dt.Rows.Cast<DataRow>().Select(x => x[0].ObjToStr()).ToList();
                }
        ...
        /// <returns></returns>
        public static List<string> GetDtFields(DbLinkArgs dbLinkArgs, string tableName)
        {
            string errMsg = string.Empty;
            string sql = string.Empty;
            if (dbLinkArgs.DbType == DatabaseType.MsSqlServer)
            {
                sql = string.Format("SELECT name FROM syscolumns WHERE id=Object_Id('{0}') order by name", tableName);
                DataTable dt = CommonOperate.ExecuteQuery(out errMsg, sql, null, dbLinkArgs.ConnString, dbLinkArgs.DbType);
                if (dt != null && dt.Rows.Count > 0)
                {
                    return dt.Rows.Cast<DataRow>().Select(x => x[0].ObjToStr()).ToList();
        ...
        /// <param name="dbLinkArgs">连接对象</param>
        /// <returns></returns>
        public static bool CreateDb(DbLinkArgs dbLinkArgs)
        {
            ...
            if (dbLinkArgs.DbType == DatabaseType.MsSqlServer)
            {
                ...
                string connStr = string.Format("Data Source={0};Initial Catalog=master;User ID={1};Password={2};Pooling=true;MAX Pool Size=512;Min Pool Size=50;Connection Lifetime=30", dbLinkArgs.DataSource, dbLinkArgs.UserId, dbLinkArgs.Pwd);
                int rs = CommonOperate.ExecuteNonQuery(out errMsg, createDbSb.ToString(), null, connStr);
            ...
            else
            {
                ...
                int rs = CommonOperate.ExecuteNonQuery(out errMsg, createDbSb.ToString(), null, connStr, dbLinkArgs.DbType);
                return string.IsNullOrEmpty(errMsg);
            }
        }
        ...
        /// 注册链接服务器
        /// </summary>
        /// <param name="local">本地服务器连接对象</param>
        /// <param name="dbLinkArgs">需要被注册的链接服务器对象</param>
        /// <returns></returns>
        public static bool RegCrossLinkServer(DbLinkArgs local, DbLinkArgs dbLinkArgs)
        {
            if (local == null || string.IsNullOrEmpty(local.ConnString) || dbLinkArgs == null)
                return false;
            string errMsg = string.Empty;
            string sql = string.Empty;
            if (dbLinkArgs.DbType == DatabaseType.MsSqlServer)
                sql = string.Format("exec sp_dropserver '{0}','droplogins';exec sp_addlinkedserver  '{0}','','SQLOLEDB','{0}';exec sp_addlinkedsrvlogin '{0}','false',null,'{1}','{2}'", dbLinkArgs.DataSource, dbLinkArgs.UserId, dbLinkArgs.Pwd);
            int rs = CommonOperate.ExecuteNonQuery(out errMsg, sql, null, local.ConnString, local.DbType);
            return rs > 0;
        }
        ...
        private static void RegStoredProcedure(DbLinkArgs dbLinkArgs)
        {
            ...
            if (dbLinkArgs.DbType == DatabaseType.MsSqlServer)
            {
                ...
                if (!string.IsNullOrWhiteSpace(dbLinkArgs.DbName))
                {
                    ...
                    createDbSb.AppendLine(" (");
                    createDbSb.AppendLine(" 	NAME='" + dbLinkArgs.DbName + "',");
                    createDbSb.AppendLine(" 	FILENAME='" + appDataPath + Path.DirectorySeparatorChar.ToString() + dbLinkArgs.DbName + ".mdf',");
                    createDbSb.AppendLine(" 	FileGrowth=1MB");
                    createDbSb.AppendLine(" )");
                    createDbSb.AppendLine(" LOG ON --创建日志文件");
                    createDbSb.AppendLine(" (");
                    createDbSb.AppendLine(" 	NAME='HkTempLog',");
                    createDbSb.AppendLine("	    FileName='" + appDataPath + Path.DirectorySeparatorChar.ToString() + dbLinkArgs.DbName + ".ldf',");
                    createDbSb.AppendLine("	    FileGrowth=1MB");
                    createDbSb.AppendLine(" )");
                    createDbSb.AppendLine(" END");
                    string connStr = string.Format("Data Source={0};Initial Catalog=master;User ID={1};Password={2};Pooling=true;MAX Pool Size=512;Min Pool Size=50;Connection Lifetime=30", dbLinkArgs.DataSource, dbLinkArgs.UserId, dbLinkArgs.Pwd);
        ...
        {
            Bridge.BridgeObject.GetAllModelTypes(); //先缓存所有model类型
            string errMsg = string.Empty;
        ...
        /// <param name="tempTitle">重写后的标题</param>
        /// <param name="tempContent">重写后的内容</param>
        private static void TriggerEventNotify(List<Msg_EventNotify> eventNotifys, List<Guid> recordIds, UserInfo currUser, Dictionary<string, string> otherDicTos = null, Dictionary<string, string> otherDicCcs = null, Dictionary<string, string> otherDicBccs = null, string sendFlag = null, string tempTitle = null, string tempContent = null)
        {
            ...
            List<Sys_Field> sysFields = SystemOperate.GetFieldInfos(moduleId);
            foreach (Msg_EventNotify obj in eventNotifys)
            {
                ...
                foreach (Guid recordId in recordIds)
                {
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        ...
                        {
                            string needReplace = "{" + sysField.Name + "}";
                            if (title.Contains(needReplace) || content.Contains(needReplace))
                            {
                                string fieldValue = SystemOperate.GetFieldDisplayValue(moduleId, recordId, sysField.Name);
        ...
        public static void TriggerEventNotify(Guid moduleId, Guid recordId, Guid workNodeId, WorkActionEnum workAction, UserInfo currUser, Guid? starterEmpId, Dictionary<Guid, Bpm_WorkToDoList> nextHandlers, WorkFlowStatusEnum flowStatus, Guid? directHandler, Bpm_WorkToDoList workTodo)
        {
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                else if (list.Count > 0) //当前节点已设置了模板
                {
                    ...
                    if (canEvent)
                    {
                        List<Msg_EventNotify> msgNotifys = list.Where(x => x.NotifyTypeOfEnum == EventNotifyTypeEnum.Email || x.NotifyTypeOfEnum == EventNotifyTypeEnum.EmailSms || x.NotifyTypeOfEnum == EventNotifyTypeEnum.EmailSys).ToList();
                        if (msgNotifys.Count() == 1) //邮件模板只有一个时
                        {
                            Dictionary<string, string> otherDicTos = new Dictionary<string, string>(); //需要主送人员
                            ...
                            OrgM_Emp empStarter = starterEmpId.HasValue ? OrgMOperate.GetEmp(starterEmpId.Value) : null;
                            bool isNotify = flowStatus == WorkFlowStatusEnum.Over || flowStatus == WorkFlowStatusEnum.Refused;
                            object[] args = new object[] { recordId, workNode.Name, workAction, flowStatus, currUser, empStarter, null, null, isNotify, otherDicCcs, subjectContens, otherDicTos };
                            ...
                            {
                                if (subjectContens != null && subjectContens.Count > 2 && subjectContens[2] == "true") //以没有模板的通用方式发送邮件
                                {
                                    if (flowStatus != WorkFlowStatusEnum.Over && flowStatus != WorkFlowStatusEnum.Refused) //通知下一审批人
                                    {
                                        WorkTodoHandleCommonEmail(moduleId, recordId, nextHandlers, workNode, workAction, empStarter, currUser, directHandler, flowStatus, workTodo.Title);
                                ...
                                else //以有邮件模板方式走通用发邮件
                                {
                                    ...
                                    TriggerEventNotify(list, new List<Guid>() { recordId }, currUser, otherDicTos, otherDicCcs, null, sendFlag, subject, content);
                                }
                            }
                        }
                        else
        ...
        private static void WorkTodoHandleCommonEmail(Guid moduleId, Guid recordId, Dictionary<Guid, Bpm_WorkToDoList> nextHandlers, Bpm_WorkNode workNode, WorkActionEnum workAction, OrgM_Emp empStarter, UserInfo currUser, Guid? directHandler, WorkFlowStatusEnum flowStatus, string todoTitle)
        {
            ...
            List<string> subjectContens = new List<string>(); //主题内容,List[0]为主题，List[1]为内容
            object returnObj = CommonOperate.ExecuteCustomeOperateHandleMethod(moduleId, "WorkflowMsgNotify", new object[] { recordId, workNode.Name, workAction, flowStatus, currUser, empStarter, nextHandlerReturns, notifyUrl, true, null, subjectContens, null });
            if (returnObj == null) //继续走通用处理
            {
        ...
        private static void WorkTodoNotifyCommonEmail(Guid moduleId, Guid recordId, Bpm_WorkNode workNode, WorkActionEnum workAction, OrgM_Emp empStarter, UserInfo currUser, WorkFlowStatusEnum flowStatus, Bpm_WorkToDoList workTodo)
        {
            ...
            Dictionary<string, string> otherDicCcs = new Dictionary<string, string>(); //需要抄送人员
            List<string> subjectContens = new List<string>(); //主题内容,List[0]为主题，List[1]为内容
            object[] args = new object[] { recordId, workNode.Name, workAction, flowStatus, currUser, empStarter, null, url, true, otherDicCcs, subjectContens, null };
            object returnObj = CommonOperate.ExecuteCustomeOperateHandleMethod(moduleId, "WorkflowMsgNotify", args);
            if (returnObj == null) //继续走通用处理
            {
                string titleKeyDisplay = SystemOperate.GetModuleTitleKeyDisplay(moduleId);
