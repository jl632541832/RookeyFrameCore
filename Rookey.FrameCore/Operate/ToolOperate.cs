        ...
        public static string InitData()
        {
            ...
            if (IsNeedInit()) //需要初始化
            {
                ...
                if (string.IsNullOrEmpty(errMsg))
                {
                    ...
                    Sys_Menu roleDataPowerConfigMenu = new Sys_Menu()
                    {
                        Name = "数据权限配置",
                        Display = "数据权限配置",
                        Sys_ModuleId = CommonOperate.GetEntity<Sys_Module>(x => x.Name == "数据权限配置", null, out errMsg).Id,
                        ParentId = permissionSetMenuId,
                        IsValid = false,
                        IsLeaf = true,
                        Sort = 2
                    };
                    Guid roleDataPowerConfigMenuId = CommonOperate.OperateRecord<Sys_Menu>(roleDataPowerConfigMenu, OperateHandle.ModelRecordOperateType.Add, out errMsg, null, false);
                    ...
                    if (!isOnlyInitSys)
                    {
                        ...
                        #endregion
                        #region 组织机构初始化
                        List<string> orgMTables = new List<string>() { "OrgM_Dept", "OrgM_DeptDuty", "OrgM_Duty", "OrgM_Emp", "OrgM_EmpDeptDuty" };
                        CommonOperate.UpdateRecordsByExpression<Sys_Module>(new { IsEnableCodeRule = true }, x => orgMTables.Contains(x.TableName), out errMsg);
                        List<Sys_BillCodeRule> codeRules = new List<Sys_BillCodeRule>();
                        ...
                        List<string> codeRuleNames = new List<string>() { "部门管理编码规则", "职务管理编码规则", "岗位管理编码规则", "员工管理编码规则", "员工岗位编码规则" };
                        CommonOperate.DeleteRecordsByExpression<Sys_BillCodeRule>(x => codeRuleNames.Contains(x.Name), out errMsg);
                        CommonOperate.OperateRecords<Sys_BillCodeRule>(codeRules, ModelRecordOperateType.Add, out errMsg, false);
                        #endregion
                    }
                    #endregion
                    #region 自定义初始化
