...
namespace Rookey.Frame.Operate.Base
{
    ...
    public static class CommonOperate
    {
        ...
        public class TempOperate<T> where T : BaseEntity
        {
                ...
                foreach (PropertyInfo p in ps)
                {
                    ...
                    try
                    {
                        if (!fieldAttr.IsMultiSelect || p.PropertyType != typeof(String)) //单选外键
                        {
                                ...
                                if (references && navFieldName == "Parent") //Parent导航属性处理
                                {
                                    if (foreignKeyIds.Count > 1)
                                    {
                                        ...
                                        foreach (object obj in (navObjs as IEnumerable))
                                        {
                                            ...
                                            Guid tempId = (obj as BaseEntity).Id;
                                            if (!tempDic.ContainsKey(tempId))
                                                tempDic.Add(tempId, obj);
                                        }
                                        if (!foreignNavList.ContainsKey(navProperty))
                            ...
                            if (!foreignNavDic.ContainsKey(temp))
                                foreignNavDic.Add(temp, p);
                            //取外键集合的值
                            QueryMethod queryMethod = foreignKeyIds.Count == 1 ? QueryMethod.Equal : QueryMethod.In;
                            object val = foreignKeyIds.FirstOrDefault();
                            if (foreignKeyIds.Count > 1)
                                val = foreignKeyIds.ToArray();
                            List<ConditionItem> cdItems = new List<ConditionItem>() { new ConditionItem() { Field = "Id", Method = queryMethod, Value = val } };
                            object lamdaExp = CommonOperate.GetQueryCondition(moduleConfig.Name, cdItems);
                            DatabaseType dbType = DatabaseType.MsSqlServer;
                            string connStr = ModelConfigHelper.GetModelConnStr(foreighModelType, out dbType);
                            object foreignList = CommonOperate.GetEntities(foreighModelType.Name, out errMsg, lamdaExp, null, false, null, null, null, new List<string>() { "Id", moduleConfig.TitleKey }, false, connStr, dbType);
                            if (foreignList != null)
                            {
                                PropertyInfo fIdP = foreighModelType.GetProperty("Id");
                                PropertyInfo fTitleKeyP = foreighModelType.GetProperty(moduleConfig.TitleKey);
                                Dictionary<Guid, object> tempDic = new Dictionary<Guid, object>();
                                foreach (object foreignObj in (foreignList as IEnumerable))
                                {
                                    Guid foreignId = fIdP.GetValue2(foreignObj, null).ObjToGuid();
                                    object foreignTk = fTitleKeyP.GetValue2(foreignObj, null);
                                    if (!tempDic.ContainsKey(foreignId))
                                        tempDic.Add(foreignId, foreignTk);
                                }
                                if (!foreignNavList.ContainsKey(temp))
                                    foreignNavList.Add(temp, tempDic);
                            }
                            #endregion
                        }
                        else //多选外键
                        {
                            ...
                            //取外键集合的值
                            List<ConditionItem> cdItems = new List<ConditionItem>() { new ConditionItem() { Field = "Id", Method = QueryMethod.In, Value = foreignKeyIds.ToArray() } };
                            object lamdaExp = CommonOperate.GetQueryCondition(moduleConfig.Name, cdItems);
                            DatabaseType dbType = DatabaseType.MsSqlServer;
                            string connStr = ModelConfigHelper.GetModelConnStr(foreighModelType, out dbType);
                            object foreignList = CommonOperate.GetEntities(foreighModelType.Name, out errMsg, lamdaExp, null, false, null, null, null, new List<string>() { "Id", moduleConfig.TitleKey }, false, connStr, dbType);
                            if (foreignList != null)
                            {
                                PropertyInfo fIdP = foreighModelType.GetProperty("Id");
                                PropertyInfo fTitleKeyP = foreighModelType.GetProperty(moduleConfig.TitleKey);
                                Dictionary<Guid, string> tempForeignDic = new Dictionary<Guid, string>();
                                foreach (object foreignObj in (foreignList as IEnumerable))
                                {
                                    Guid foreignId = fIdP.GetValue2(foreignObj, null).ObjToGuid();
                                    string foreignTk = fTitleKeyP.GetValue2(foreignObj, null).ObjToStr();
                                    if (!tempForeignDic.ContainsKey(foreignId))
                                        tempForeignDic.Add(foreignId, foreignTk);
                                }
                                ...
                                foreach (string tempIdStr in foreignKeyIdStrs)
                                {
                                    ...
                                    string tempTextStr = string.Join(",", tempForeignDic.Where(x => tempIds.Contains(x.Key)).Select(x => x.Value));
                                    if (!tempDic.ContainsKey(tempIdStr))
                                        tempDic.Add(tempIdStr, tempTextStr);
                                }
                                if (!mutiForeignNavList.ContainsKey(temp))
            ...
            /// <param name="dbType">数据库类型</param>
            /// <returns></returns>
            public List<T> GetEntitiesByField(string fieldName, object fieldValue, out string errMsg, Expression<Func<T, bool>> expression = null, string where = null, bool permissionFilter = true, List<string> orderFields = null, List<bool> isDescs = null, int? top = null, List<string> fieldNames = null, bool references = false, string connString = null, DatabaseType? dbType = null)
            {
                errMsg = string.Empty;
                MethodCallTypeEnum methodCallType = BridgeObject.GetBLLMethodType(typeof(T), "GetEntitiesByField");
                if (methodCallType != MethodCallTypeEnum.CommonMethod)
                {
                    object[] args = new object[] { errMsg, fieldName, fieldValue, expression, where, permissionFilter, orderFields, isDescs, top, fieldNames, references, connString };
                ...
                {
                    IBaseBLL<T> bll = BridgeObject.Resolve<IBaseBLL<T>>(currUser, dbType);
                    List<T> list = bll.GetEntitiesByField(out errMsg, fieldName, fieldValue, expression, where, permissionFilter, orderFields, isDescs, top, fieldNames, references, connString);
                    if (references)
                        SetModelListForeignNameFieldValue(list, null, references);
                    return list;
                }
            ...
            {
                List<T> list = GetEntitiesByField(fieldName, fieldValue, out errMsg, null, null, permissionFilter, orderFields, isDescs, null, fieldNames, references, connString, dbType);
            ...
            public Guid OperateRecord(T t, ModelRecordOperateType operateType, out string errMsg, List<string> fieldNames = null, bool permissionValidate = true, bool references = false, string connString = null, DatabaseType? dbType = null, IDbConnection transConn = null)
            {
                ...
                bool isUpdateFlowStatus = fieldNames != null && fieldNames.Count == 1 && fieldNames.FirstOrDefault() == "FlowStatus";
                if (!isUpdateFlowStatus) //非流程状态更新
                {
                    //操作前验证
                ...
                if (transConn == null)
                {
                    if (!isUpdateFlowStatus) //非流程状态更新时
                    {
                        //执行操作完成事件
            ...
            public bool DeleteRecords(IEnumerable ids, out string errMsg, bool isSoftDel = false, bool permissionValidate = true, string connString = null, DatabaseType? dbType = null, IDbConnection transConn = null)
            {
                ...
                foreach (object id in ids)
                {
                    ...
                    if (isSoftDel)
                    {
                        ...
                        if (currUser != null)
                            t.ModifyUserId = currUser.UserId;
                    }
                    ts.Add(t as T);
                }
                //操作前验证
            ...
            public TreeNode GetTreeNode(Guid? parentId, Action<T, TreeAttributes> attributeParams = null, string pIdFieldName = "ParentId", string textName = "Name", string sortField = "Id", string icon = null, Expression<Func<T, bool>> expression = null, string connString = null, DatabaseType? dbType = null, bool isAsync = false)
            {
                ...
                if (expression == null)
                {
                    ...
                    if (!parentId.HasValue || parentId.Value == Guid.Empty) //没有指定父结点时
                    {
                        ...
                        List<T> tmpTs1 = GetEntitiesByField<T>(pidName, null, out errMsg, null, null, true, new List<string>() { sortField }, new List<bool>() { false }, null, null, false, connString, dbType, currUser);
                        ...
                        List<T> tmpTs2 = GetEntitiesByField<T>(pidName, Guid.Empty, out errMsg, null, null, true, new List<string>() { sortField }, new List<bool>() { false }, null, null, false, connString, dbType, currUser);
            ...
            public List<T> GetChildNodesData(Guid? parentId, string pIdFieldName = "ParentId", string sortField = "Id", Expression<Func<T, bool>> expression = null, string connString = null, DatabaseType? dbType = null)
            {
                string errMsg = string.Empty;
                string pidName = string.IsNullOrEmpty(pIdFieldName) ? "ParentId" : pIdFieldName;
                Expression<Func<T, bool>> exp = expression != null ? expression.And(x => x.IsDeleted == false && x.IsDraft == false) : x => x.IsDeleted == false && x.IsDraft == false;
                List<T> list = this.GetEntitiesByField(pidName, parentId, out errMsg, exp, null, true, new List<string>() { sortField }, new List<bool>() { false }, null, null, false, connString, dbType);
                return list;
            }
                            ...
                            foreach (string fieldName in q.Keys)
                            {
                                ...
                                if (tempSingleQItem == null && string.IsNullOrWhiteSpace(tempSingleQWhere)) //不存在重写单个字段搜索接口
                                {
                                    if (fieldName != "FlowStatus")
                                    {
                                        ...
                                        {
                                            formField = sysField.Sys_ModuleId.Value == module.Id ? SystemOperate.GetNfDefaultFormSingleField(module.Id, fieldName) : null;
                                        }
                                        Type fieldType = sysField == null || sysField.Sys_ModuleId.Value == module.Id ? SystemOperate.GetFieldType(module.Id, fieldName) : null;
        ...
        /// <returns></returns>
        public static object GetModelFieldValueByModel(Guid moduleId, object model, Sys_Field sysField)
        {
            if (sysField.Sys_ModuleId.HasValue && sysField.Sys_ModuleId.Value != moduleId)
            {
                string tableName = SystemOperate.GetModuleTableNameById(sysField.Sys_ModuleId.Value);
                if (!string.IsNullOrEmpty(tableName))
                {
                    Type modelType = SystemOperate.GetModelType(moduleId);
                    string temFn = string.Format("{0}Id", tableName);
                    PropertyInfo pFieldName = modelType.GetProperty(temFn);
                    if (pFieldName != null)
                    {
                        Guid recordId = pFieldName.GetValue2(model, null).ObjToGuid();
                        string errMsg = string.Empty;
                        object tempModel = CommonOperate.GetEntityById(sysField.Sys_ModuleId.Value, recordId, out errMsg);
                        if (tempModel != null)
                            return GetModelFieldValueByModel(sysField.Sys_ModuleId.Value, tempModel, sysField.Name);
                    }
                }
            }
            return GetModelFieldValueByModel(moduleId, model, sysField.Name);
        }
        ...
        /// <param name="currUser">当前用户</param>
        /// <returns></returns>
        public static List<T> GetEntitiesByField<T>(string fieldName, object fieldValue, out string errMsg, Expression<Func<T, bool>> expression = null, string where = null, bool permissionFilter = true, List<string> orderFields = null, List<bool> isDescs = null, int? top = null, List<string> fieldNames = null, bool references = false, string connString = null, DatabaseType? dbType = null, UserInfo currUser = null) where T : BaseEntity
        {
            return new TempOperate<T>(currUser).GetEntitiesByField(fieldName, fieldValue, out errMsg, expression, where, permissionFilter, orderFields, isDescs, top, fieldNames, references, connString, dbType);
        }
        ...
        /// <param name="currUser">当前用户</param>
        /// <returns></returns>
        public static object GetEntitiesByField(string tableName, string fieldName, object fieldValue, out string errMsg, object expression = null, string where = null, bool permissionFilter = true, List<string> orderFields = null, List<bool> isDescs = null, int? top = null, List<string> fieldNames = null, bool references = false, string connString = null, DatabaseType? dbType = null, UserInfo currUser = null)
        {
            errMsg = string.Empty;
            object[] args = new object[] { fieldName, fieldValue, errMsg, expression, where, permissionFilter, orderFields, isDescs, top, fieldNames, references, connString, dbType };
            //反射取数据
        ...
        /// <param name="currUser">当前用户</param>
        /// <returns></returns>
        public static object GetEntitiesByField(Guid moduleId, string fieldName, object fieldValue, out string errMsg, object expression = null, string where = null, bool permissionFilter = true, List<string> orderFields = null, List<bool> isDescs = null, int? top = null, List<string> fieldNames = null, bool references = false, string connString = null, DatabaseType? dbType = null, UserInfo currUser = null)
        {
            Sys_Module module = SystemOperate.GetModuleById(moduleId);
            if (module != null && !string.IsNullOrEmpty(module.TableName))
            {
                return GetEntitiesByField(module.TableName, fieldName, fieldValue, out errMsg, expression, where, permissionFilter, orderFields, isDescs, top, fieldNames, references, connString, dbType, currUser);
            }
            errMsg = "模块不存在或模块表名为空！";
            return null;
        }
        ...
        public static Dictionary<string, string> GetColumnFieldValues(out string errMsg, Guid moduleId, string fieldName)
        {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        return dt.Rows.Cast<DataRow>().ToDictionary(x => x[0].ObjToStr(), y => SystemOperate.GetEnumFieldDisplayText(moduleId, fieldName, y[0].ObjToStr()));
                    }
                    else
                    {
                        return dt.Rows.Cast<DataRow>().ToDictionary(x => x[0].ObjToStr(), y => y[1].ObjToStr());
                    }
                }
            }
            catch (Exception ex)
        ...
        public static List<object> GetColumnFieldValues<T>(out string errMsg, string fieldName, Expression<Func<T, bool>> exp = null) where T : BaseEntity
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
                return dt.Rows.Cast<DataRow>().Select(x => x[0]).ToList();
                ...
                if (nv.name != "Id")
                {
                    ...
                    if (formField.ControlTypeOfEnum == ControlTypeEnum.ImageUpload && !string.IsNullOrWhiteSpace(tempValue))
                    {
                        try
                        {
                            ...
                            if (sourcePath.Contains(string.Format("Upload{0}Image{0}ImgUploadControl", Path.DirectorySeparatorChar))) //图片未发生改变时不更新该字段
                                continue;
                            //目标路径
                            string desPath = Globals.GetWebDir() + string.Format("Upload{0}Image{0}ImgUploadControl{0}", Path.DirectorySeparatorChar);
                            ...
                            tempValue = "/" + desPath.Replace(Globals.GetWebDir(), "").Replace(Path.DirectorySeparatorChar.ToString(), "/");
                        }
                        catch { }
                    }
                    #endregion
                                ...
                                if (nv.name != "Id")
                                {
                                    ...
                                    if (detailFormField.ControlTypeOfEnum == ControlTypeEnum.ImageUpload && !string.IsNullOrWhiteSpace(tempValue))
                                    {
                                        try
                                        {
                                            //图片上传控件处理真实的图片地址
                                            //源路径
                                            string sourcePath = Globals.GetWebDir() + tempValue.Substring(1, tempValue.Length - 1).Replace("/", Path.DirectorySeparatorChar.ToString());
                                            if (sourcePath.Contains(string.Format("Upload{0}Image{0}ImgUploadControl", Path.DirectorySeparatorChar))) //图片未发生改变时不更新该字段
                                                continue;
                                            //目标路径
                                            string desPath = Globals.GetWebDir() + string.Format("Upload{0}Image{0}ImgUploadControl{0}", Path.DirectorySeparatorChar);
                ...
                foreach (FormFastObjectDetail obj in formObj.Details)
                {
                    ...
                    if (obj.ModuleDatas != null && obj.ModuleDatas.Count > 0)
                    {
                        ...
                        if (addDetails.Count > 0)
                        {
                            ...
                            {
                                string billFieldName = SystemOperate.GetBillCodeFieldName(detailModule); //编码字段
                                PropertyInfo billFieldProperty = detailModelType.GetProperty(billFieldName);
                                if (billFieldProperty.PropertyType == typeof(String))
            ...
            if (verifyFields != null && verifyFields.Count > 0)
            {
                foreach (string field in verifyFields)
                {
                    ...
                    if (formField.IsRequired.HasValue && formField.IsRequired.Value) //必填验证
                    {
                        ...
                        if (value == null || value.ObjToStr() == string.Empty || (isForeignField && field.EndsWith("Id") && value.ObjToGuid() == Guid.Empty))
                        {
        ...
        public static IEnumerable ImportCheck(Guid moduleId, Guid userId, string importFile, out string errMsg)
        {
            ...
            {
                var fs = new FileStream(importFile, System.IO.FileMode.Open);
                DataTable dt = null; //NPOI_ExcelHelper.RenderFromExcel(fs);
        ...
        /// <param name="currUser">当前用户</param>
        /// <returns></returns>
        public static Dictionary<string, string> GetConditionDic(string condition, UserInfo currUser = null)
        {
        ...
        public static object GetTempGridData(GridDataParmas gridDataParmas, out long total, UserInfo currUser, object otherLamdaExpress = null, bool isReturnDtType = false, bool isFieldDipalyDes = false)
        {
            ...
            //字段重写处理
            GridParams gridParams = new GridParams() { GridFields = gridFields };
            CommonOperate.ExecuteCustomeOperateHandleMethod(gridDataParmas.ModuleId, "GridParamsSet", new object[] { gridDataParmas.GridType, gridParams, gridDataParmas.RequestObj });
            if (gridParams.GridFields != null && gridParams.GridFields.Count > 0)
                gridFields = gridParams.GridFields;
            //如果是数据导出
            if (gridDataParmas.IsExportData)
            {
            ...
            string exportServer = WebConfigHelper.GetAppSettingValue("ExportServer");
            if (gridDataParmas.IsExportData)
            {
                if (!string.IsNullOrEmpty(exportServer))
                {
                    DbLinkArgs dbLinkArgs = ModelConfigHelper.GetDbLinkArgs(connStr, dbType);
                    if (dbLinkArgs != null)
                    {
                        dbLinkArgs.DataSource = exportServer;
                        if (SystemOperate.DbIsExists(dbLinkArgs)) //数据库存在时
                        {
                            connStr = ModelConfigHelper.GetDbLinkArgsConnStr(dbLinkArgs);
                        }
                    }
                }
            }
            ...
            //查询数据
            //调用自定义取数据源方法
            object[] args = new object[] { total, tableName, selectFields, whereSql, gridDataParmas.PagingInfo, gridDataParmas.IsExportData };
            object viewData = CommonOperate.ExecuteCustomeOperateHandleMethod(gridDataParmas.ModuleId, "GetViewOrExportData", args);
            DataTable dt = null;
            if (viewData == null)
            {
                //走通用处理方法
                dt = PagingQueryByProcedure(out errMsg, out total, tableName, selectFieldsStr, whereSql, gridDataParmas.PagingInfo, connStr, dbType);
                if (dt == null) return null;
                if (!isReturnDtType || queryFields.Values.Select(x => x.Sys_ModuleId.Value).Count() > 1) //返回实体集合或涉及到主从模块和外键模块时要处理字段
                {
                    #region 字段加工处理
                            ...
                            try
                            {
                                ...
                                if (fns.Contains(fieldName)) //当前模块或明细模块
                                {
                                    ...
                                    else if (!string.IsNullOrEmpty(sysField.ForeignModuleName) && fieldName.Length > 4 && fieldName.Substring(0, fieldName.Length - 4) + "Id" == sysField.Name) //外键Name字段
                                    {
                                        ...
                                        {
                                            Type tempModelType = CommonOperate.GetModelType(foreignModule.TableName);
                                            DatabaseType tempDbType = DatabaseType.MsSqlServer;
                                            string tempConnStr = ModelConfigHelper.GetModelConnStr(tempModelType, out tempDbType);
                                            if (gridDataParmas.IsExportData && !string.IsNullOrEmpty(exportServer))
                                            {
                                                DbLinkArgs tempDbLinkArgs = ModelConfigHelper.GetDbLinkArgs(tempConnStr, tempDbType);
                                                if (tempDbLinkArgs != null)
                                                {
                                                    tempDbLinkArgs.DataSource = exportServer;
                                                    if (SystemOperate.DbIsExists(tempDbLinkArgs)) //数据库存在时
                                                    {
                                                        tempConnStr = ModelConfigHelper.GetDbLinkArgsConnStr(tempDbLinkArgs);
                                                    }
                                                }
                                            }
                                ...
                                else if (sysField.Sys_ModuleId.Value == initModuleId) //存在明细模块，当前是主模块
                                {
                                    ...
                                    DatabaseType tempDbType = DatabaseType.MsSqlServer;
                                    string tempConnStr = ModelConfigHelper.GetModelConnStr(initModelType, out tempDbType);
                                    if (gridDataParmas.IsExportData && !string.IsNullOrEmpty(exportServer))
                                    {
                                        DbLinkArgs tempDbLinkArgs = ModelConfigHelper.GetDbLinkArgs(tempConnStr, tempDbType);
                                        if (tempDbLinkArgs != null)
                                ...
                                else //主模块的外键模块
                                {
                                    ...
                                    if (foreignId != Guid.Empty)
                                    {
                                        ...
                                        Type tempModelType = CommonOperate.GetModelType(sysField.Sys_ModuleId.Value);
                                        DatabaseType tempDbType = DatabaseType.MsSqlServer;
                                        string tempConnStr = ModelConfigHelper.GetModelConnStr(tempModelType, out tempDbType);
                                        if (gridDataParmas.IsExportData && !string.IsNullOrEmpty(exportServer))
                                        {
            ...
            {
                total = args[0].ObjToLong();
            }
            if (isReturnDtType) //返回datatable
            {
                if (isFieldDipalyDes) //列头显示字段描述
                {
                    if (viewData != null) //自定义数据源
                    {
                        tempDt = viewData as DataTable;
                    }
                    if (tempDt != null)
                    {
                        for (int i = 0; i < tempDt.Columns.Count; i++)
                        {
                            ...
                            else if (viewData != null && columnName.EndsWith("Name")) //自定义数据源并且为外键Name字段
                            {
                                string idField = columnName.StartsWith("Other") ? columnName.Substring(0, columnName.Length - 4) : columnName.Substring(0, columnName.Length - 4) + "Id";
                                if (queryFields.ContainsKey(idField) && !string.IsNullOrEmpty(queryFields[idField].ForeignModuleName))
                                {
                                    tempDt.Columns[i].ColumnName = queryFields[idField].Display;
                                }
                            }
                        }
                    }
                }
                if (gridDataParmas.IsExportData && viewData == null) //导出数据
                {
                    //调用自定义数据处理接口
                    object tempData = ExecuteCustomeOperateHandleMethod(moduleId, "ViewGridDataHandle", new object[] { tempDt, currUser, dt });
                    if (tempData != null)
                    {
                        tempDt = (DataTable)tempData;
                    }
                }
                return tempDt;
            }
            return viewData == null ? objList : viewData;
            #endregion
        }
        ...
        /// <returns></returns>
        public static GridDataParmas GetGridDataParams(HttpRequest request)
        {
            //模块信息
            ...
            DbLinkArgs dblinkArgs = ModelConfigHelper.GetDbLinkArgs(modelType);
            if (dblinkArgs == null) dblinkArgs = ModelConfigHelper.GetLocalDbLinkArgs();
            bool isMysql = dblinkArgs != null && dblinkArgs.DbType == DatabaseType.MySql; //mysql数据库
            string defaultSortField = isEnableCache || ModelConfigHelper.ModelIsViewMode(modelType) || isMysql ? "CreateDate" : null;
            //分页信息
            ...
            //搜索条件
            string q = request.QueryEx("q").ObjToStr().Trim();
            //if (!string.IsNullOrEmpty(q) && !isEnableCache && (string.IsNullOrEmpty(pageInfo.sortname) || pageInfo.sortname == "CreateDate"))
            //{
            ...
            //}
            //自定义条件
            string conditionStr = request.QueryEx("condition").ObjToStr();
            string condition = HttpUtility.UrlDecode(conditionStr);
            //复杂条件集合
            string tempCdItemStr = request.QueryEx("cdItems").ObjToStr();
            string cdItemStr = HttpUtility.UrlDecode(tempCdItemStr);
            ...
            try
            {
                string tempWhere = request.QueryEx("where").ObjToStr();
                if (!string.IsNullOrWhiteSpace(tempWhere))
                {
                    whereCon = MySecurity.DecodeBase64(tempWhere).Replace("%20", " ");
                    whereCon = HttpUtility.UrlDecode(whereCon);
                }
            }
            catch
            { }
            //弹出框的原始模块
            string initModuleStr = request.QueryEx("initModule").ObjToStr();
            string initModule = HttpUtility.UrlDecode(initModuleStr);
            //弹出框的原始字段
            string initField = request.QueryEx("initField").ObjToStr();
            //弹出框的依赖字段值
            string relyFieldsValue = string.Empty; //HttpUtility.UrlDecode(request.QueryEx()["p_relyValues"].ObjToStr());
            //自动完成字段名
            string fieldName = request.QueryEx("fieldName").ObjToStr(); //字段名
            //组装参数对象
            ...
            try
            {
                string gt = request.QueryEx("tgt").ObjToStr();
                gridParams.GridType = (DataGridType)Enum.Parse(typeof(DataGridType), gt);
            }
            catch { }
            //其他参数
            if (request.QueryExDic().Keys.Where(x => x.StartsWith("p_")).Count() > 0)
            {
                gridParams.OtherParams = new Dictionary<string, string>();
                List<string> keys = request.QueryExDic().Keys.Where(x => x.StartsWith("p_")).ToList();
                foreach (string key in keys)
                {
                    if (!gridParams.OtherParams.ContainsKey(key))
                        gridParams.OtherParams.Add(key, request.QueryExDic()[key].ObjToStr());
                }
            }
            //行过滤规则解析
            string filterRules = request.QueryEx("filterRules").ObjToStr();
            ...
            string viewIdStr = request.QueryEx("viewId").ObjToStr();
            gridParams.ViewId = viewIdStr.ObjToGuidNull(); //综合视图Id
            string gvIdStr = request.QueryEx("gvId").ObjToStr();
            gridParams.GridViewId = gvIdStr.ObjToGuidNull(); //当前加载视图ID
            string dvStr = request.QueryEx("dv").ObjToStr();
            gridParams.IsComprehensiveDetailView = dvStr.ObjToInt() == 1; //综合明细视图
            string copyStr = request.QueryEx("copy").ObjToStr();
            gridParams.IsDetailCopy = copyStr.ObjToInt() == 1; //是否明细复制
            string tgStr = request.QueryEx("tg").ObjToStr();
            gridParams.IsTreeGrid = tgStr.ObjToInt() == 1; //是否为树型网格
            string nfpStr = request.QueryEx("nfp").ObjToStr();
            gridParams.IsPermissionFilter = nfpStr.ObjToInt() != 1; //是否过滤权限
            string rsfStr = request.QueryEx("rsf").ObjToStr();
            gridParams.IsRestartFlow = rsfStr.ObjToInt() == 1; //是否为重新发起流程
            gridParams.RequestObj = request;
            //调用自定义设置参数方法
