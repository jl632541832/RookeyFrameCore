...
namespace Rookey.Frame.Operate.Base.OperateHandle
{
    ...
    public interface IGridOperateHandle<T> where T : class
    {
        ...
        void GridParamsSet(DataGridType gridType, GridParams gridParams, HttpRequest request = null);
        ...
        /// <param name="request">请求对象</param>
        void GridLoadDataParamsSet(Sys_Module module, GridDataParmas gridDataParams, HttpRequest request = null);
    ...
    public interface IFlowOperateHandle<T> where T : class
    {
        ...
        /// <param name="nextNodeName">下一审批节点</param>
        void AfterFlowOperateCompleted(Guid id, string currNodeName, UserInfo currUser, bool isOpSuccess, WorkActionEnum workAction, WorkFlowStatusEnum flowStatus, string approvalOpinions, string nextNodeName);
        ...
        /// <returns></returns>
        List<Guid> GetNodeHandler(Guid recordId, string nodeName, Guid workFlowInstId, UserInfo currUser, out string cErrMsg);
    ...
    public interface IMsgNotifyHandle<T> where T : class
    {
        ...
        /// <param name="otherDicCcs">需要知会的其他抄送人员，主要针对邮件通知</param>
        /// <param name="subjectContens">主题内容，List[0]为主题，List[1]为内容</param>
        /// <param name="otherDicTos">需要知会的其他主送人员，主要针对邮件通知</param>
        /// <returns>返回null继续走通用处理，否则表示处理成功</returns>
        string WorkflowMsgNotify(Guid recordId, string nodeName, WorkActionEnum workAction, WorkFlowStatusEnum flowStatus, UserInfo currUser, OrgM_Emp empStarter, Dictionary<Guid, string> nextHandlers, string notifyUrl, bool isEmailNotify = true, Dictionary<string, string> otherDicCcs = null, List<string> subjectContens = null, Dictionary<string, string> otherDicTos = null);
    ...
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public interface IOtherHandle<T> where T : class
    {
        #region 其他接口
        /// <summary>
        /// 返回分页视图网格或导出数据前对数据处理
        /// </summary>
        /// <param name="data">处理前的网格数据，如果是视图则是List对象，如果是导出则是DataTable</param>
        /// <param name="currUser">当前用户</param>
        /// <param name="otherObj">其他对象</param>
        object ViewGridDataHandle(object data, UserInfo currUser = null, object otherObj = null);

        /// <summary>
        /// 自定义获取编码规则
        /// </summary>
        /// <param name="model">实体对象</param>
        /// <param name="currCode">当前编码，多条记录同时保存时要根据当前编码获取下一编码</param>
        /// <param name="currUser">当前用户</param>
        /// <returns></returns>
        string GetBillCode(object model, string currCode, UserInfo currUser);
        ...
        /// <param name="currUser">当前用户</param>
        /// <returns></returns>
        bool UpdateBillCode(string currCode, UserInfo currUser);
        #endregion
    }
    ...
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public interface IParamReplaceHandle<T> where T : class
    {
        #region 参数替换
        /// <summary>
        /// 替换角色数据权限自定义参数
        /// </summary>
        /// <param name="powerSql">权限where语句</param>
        /// <param name="currUser">当前用户</param>
        /// <returns></returns>
        string ReplaceRoleDataPowerCustomerParams(string powerSql, UserInfo currUser);
        #endregion
    }
    ...
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public interface IViewGridHandle<T> where T : class
    {
        #region 综合视图或数据导出数据源处理
        /// <summary>
        /// 综合视图或数据导出数据源处理
        /// </summary>
        /// <param name="total">总记录数</param>
        /// <param name="tableName">当前查询表</param>
        /// <param name="selectFields">查询字段</param>
        /// <param name="whereSql">条件语句</param>
        /// <param name="pageInfo">分页对象</param>
        /// <param name="isExportData">是否为导出数据</param>
        /// <returns></returns>
        object GetViewOrExportData(out long total, string tableName, List<string> selectFields, string whereSql, PageInfo pageInfo, bool isExportData);
        #endregion
    }
    ...
    internal class OperateHandleFactory<T> where T : class
    {
        ...
        /// <param name="gridParams">网格参数对象</param>
        /// <param name="request">请求对象</param>
        internal void GridParamsSet(DataGridType gridType, GridParams gridParams, HttpRequest request = null)
        {
        ...
        /// <param name="gridDataParams">数据加载参数</param>
        /// <param name="request">请求对象</param>
        internal void GridLoadDataParamsSet(Sys_Module module, GridDataParmas gridDataParams, HttpRequest request = null)
        {
            object instance = GetOperateHandleInstance(OperateInterfaceType.GridOperate);
            if (instance != null)
            {
        ...
        /// <param name="approvalOpinions">审批意见</param>
        /// <param name="nextNodeName">下一审批节点</param>
        internal void AfterFlowOperateCompleted(Guid id, string currNodeName, UserInfo currUser, bool isOpSuccess, WorkActionEnum workAction, WorkFlowStatusEnum flowStatus, string approvalOpinions, string nextNodeName)
        {
            object instance = GetOperateHandleInstance(OperateInterfaceType.FlowOperate);
            if (instance != null)
            {
                ...
                Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ((IFlowOperateHandle<T>)instance).AfterFlowOperateCompleted(id, currNodeName, currUser, isOpSuccess, workAction, flowStatus, approvalOpinions, nextNodeName);
                    }
                    catch { }
                });
            }
        ...
        /// <param name="workFlowInstId">流程实例ID</param>
        /// <param name="currUser">当前用户</param>
        /// <param name="cErrMsg">找不到人时自定义提示信息</param>
        /// <returns></returns>
        internal List<Guid> GetNodeHandler(Guid recordId, string nodeName, Guid workFlowInstId, UserInfo currUser, out string cErrMsg)
        {
            ...
            if (instance != null)
            {
                try
                {
                    return ((IFlowOperateHandle<T>)instance).GetNodeHandler(recordId, nodeName, workFlowInstId, currUser, out cErrMsg);
                }
                catch { }
            }
            return null;
        ...
        /// <param name="isEmailNotify">是否邮件通知，否则为短信通知</param>
        /// <param name="otherDicCcs">其他需要知会的抄送人员，主要针对邮件通知</param>
        /// <param name="subjectContens">主题内容，List[0]为主题，List[1]为内容</param>
        /// <param name="otherDicTos">需要知会的其他主送人员，主要针对邮件通知</param>
        /// <returns>返回null继续走通用处理，否则表示处理成功</returns>
        internal string WorkflowMsgNotify(Guid recordId, string nodeName, WorkActionEnum workAction, WorkFlowStatusEnum flowStatus, UserInfo currUser, OrgM_Emp empStarter, Dictionary<Guid, string> nextHandlers, string notifyUrl, bool isEmailNotify = true, Dictionary<string, string> otherDicCcs = null, List<string> subjectContens = null, Dictionary<string, string> otherDicTos = null)
        {
            ...
            if (instance != null)
            {
                try
                {
                    return ((IMsgNotifyHandle<T>)instance).WorkflowMsgNotify(recordId, nodeName, workAction, flowStatus, currUser, empStarter, nextHandlers, notifyUrl, isEmailNotify, otherDicCcs, subjectContens, otherDicTos);
                }
                catch { }
            }
            return null;
        ...
        /// <param name="currUser">当前用户</param>
        /// <param name="otherObj">其他对象</param>
        internal object ViewGridDataHandle(object data, UserInfo currUser = null, object otherObj = null)
        {
            try
            {
                object instance = GetOperateHandleInstance(OperateInterfaceType.Other);
                if (instance != null)
                {
                    return ((IOtherHandle<T>)instance).ViewGridDataHandle(data, currUser, otherObj);
                }
            }
            catch { }
            return null;
        ...
        /// <param name="currUser">当前用户</param>
        /// <returns></returns>
        internal string GetBillCode(object model, string currCode, UserInfo currUser)
        {
            try
            {
                object instance = GetOperateHandleInstance(OperateInterfaceType.Other);
                if (instance != null)
                {
                    return ((IOtherHandle<T>)instance).GetBillCode(model, currCode, currUser);
                }
            }
            catch { }
            return null;
        ...
        /// <param name="currUser">当前用户</param>
        /// <returns></returns>
        internal bool UpdateBillCode(string currCode, UserInfo currUser)
        {
            try
            {
                object instance = GetOperateHandleInstance(OperateInterfaceType.Other);
                if (instance != null)
                {
                    return ((IOtherHandle<T>)instance).UpdateBillCode(currCode, currUser);
                }
            }
            catch { }
            return false;
        ...
        /// <param name="currUser">当前用户</param>
        /// <returns></returns>
        internal string ReplaceRoleDataPowerCustomerParams(string powerSql, UserInfo currUser)
        {
            try
            {
                object instance = GetOperateHandleInstance(OperateInterfaceType.ParamReplace);
                if (instance != null)
                {
                    return ((IParamReplaceHandle<T>)instance).ReplaceRoleDataPowerCustomerParams(powerSql, currUser);
                }
            }
            catch { }
            return powerSql;
        ...
        /// <param name="whereSql">条件语句</param>
        /// <param name="pageInfo">分页对象</param>
        /// <param name="isExportData">是否是导出数据</param>
        /// <returns></returns>
        internal object GetViewOrExportData(out long total, string tableName, List<string> selectFields, string whereSql, PageInfo pageInfo, bool isExportData)
        {
            total = 0;
            try
            {
                object instance = GetOperateHandleInstance(OperateInterfaceType.ViewGrid);
                if (instance != null)
                {
                    return ((IViewGridHandle<T>)instance).GetViewOrExportData(out total, tableName, selectFields, whereSql, pageInfo, isExportData);
                }
            }
            catch { }
            return null; ;
    ...
    public interface IUserOperateHandle
    {
        ...
        void AfterLoginSuccess(HttpRequest request, HttpResponse response, string username, string pwd, int expires);
    ...
    internal class UserOperateHandleFactory
    {
        ...
        /// <param name="pwd">密码</param>
        /// <param name="expires">有效时间（分钟）</param>
        public void AfterLoginSuccess(HttpRequest request, HttpResponse response, string username, string pwd, int expires)
        {
            ...
            if (instances.Count > 0)
            {
                ...
                {
                    instance.AfterLoginSuccess(request, response, username, pwd, expires);
                }
            }
        }
