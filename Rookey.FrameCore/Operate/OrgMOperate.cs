    ...
    public static class OrgMOperate
    {
        ...
        public static OrgM_Dept GetEmpLevelDepthDept(int levelDepth, Guid empId, Guid? companyId = null, Guid? deptId = null)
        {
            ...
            if (deptId.HasValue && deptId.Value != Guid.Empty)
                ...
                dept = GetEmpMainDept(empId, companyId, true);
        ...
        public static TreeNode LoadDeptTree(Guid? deptId, Expression<Func<OrgM_Dept, bool>> expression = null, bool isAsync = false)
        {
            ...
            else
            {
                ...
                {
                    List<OrgM_Dept> listChilds = GetChildDepts(root.Id, false, expression); //获取子部门
                    if (expression != null)
                    {
        ...
        /// </summary>
        /// <param name="expression">条件表达式</param>
        /// <param name="isContainQuit">是否包含离职的</param>
        /// <returns></returns>
        public static List<OrgM_Emp> GetAllEmps(Expression<Func<OrgM_Emp, bool>> expression = null, bool isContainQuit = false)
        {
            string errMsg = string.Empty;
            Expression<Func<OrgM_Emp, bool>> exp = x => !x.IsDeleted && !x.IsDraft;
            if (!isContainQuit) //不包含离职的
            {
        ...
        {
            List<OrgM_Emp> emps = GetAllEmps(x => x.Id == empId, true);
        ...
        public static OrgM_Emp GetEmpByCode(string empCode)
        {
            ...
            List<OrgM_Emp> emps = GetAllEmps(x => x.Code != null && x.Code == empCode, true);
            return emps.FirstOrDefault();
        }
        ...
        public static OrgM_Emp GetEmpByEmail(string email)
        {
            ...
            List<OrgM_Emp> emps = GetAllEmps(x => x.Email != null && x.Email.ToLower() == email.ToLower(), true);
            return emps.FirstOrDefault();
        }
        ...
        public static OrgM_Emp GetEmpByEmailPrex(string emailPrex)
        {
            ...
            List<OrgM_Emp> emps = GetAllEmps(x => x.Email != null && x.Email.Contains("@") && x.Email.Substring(0, x.Email.IndexOf("@")).ToLower() == emailPrex.ToLower(), true);
            return emps.FirstOrDefault();
        }
        ...
        /// </summary>
        /// <param name="expression">条件表达式</param>
        /// <param name="isContainDeleted">是否包含删除的</param>
        /// <returns></returns>
        public static List<OrgM_EmpDeptDuty> GetAllEmpPositions(Expression<Func<OrgM_EmpDeptDuty, bool>> expression = null, bool isContainDeleted = false)
        {
            string errMsg = string.Empty;
            Expression<Func<OrgM_EmpDeptDuty, bool>> exp = x => x.IsDraft == false && x.IsValid == true;
            if (!isContainDeleted) //不包含删除的
                exp = ExpressionExtension.And(x => x.IsDeleted == false, exp);
        ...
        /// <param name="empId">员工ID</param>
        /// <param name="companyId">所属公司ID</param>
        /// <param name="isContainDeleted">是否包含软删除的</param>
        /// <returns></returns>
        public static OrgM_EmpDeptDuty GetEmpMainPosition(Guid empId, Guid? companyId = null, bool isContainDeleted = false)
        {
            List<OrgM_EmpDeptDuty> empPostions = new List<OrgM_EmpDeptDuty>();
            if (companyId.HasValue && companyId.Value != Guid.Empty)
            {
                List<Guid?> childDeptIds = GetChildDepts(companyId.Value).Select(x => (Guid?)x.Id).ToList();
                empPostions = GetAllEmpPositions(x => x.OrgM_EmpId == empId && x.IsMainDuty && childDeptIds.Contains(x.OrgM_DeptId), isContainDeleted);
            }
            else
            {
                empPostions = GetAllEmpPositions(x => x.OrgM_EmpId == empId && x.IsMainDuty, isContainDeleted);
            }
            OrgM_EmpDeptDuty empPostionNoDel = empPostions.Where(x => !x.IsDeleted).FirstOrDefault();
            if (empPostionNoDel != null)
                return empPostionNoDel;
            return empPostions.FirstOrDefault();
        }
        ...
        /// <param name="empId">员工ID</param>
        /// <param name="companyId">所属公司ID</param>
        /// <param name="isContainDeleted">是否包含已离职员工</param>
        /// <returns></returns>
        public static OrgM_Duty GetEmpMainDuty(Guid empId, Guid? companyId = null, bool isContainDeleted = false)
        {
            OrgM_EmpDeptDuty empPosition = GetEmpMainPosition(empId, companyId, isContainDeleted);
        ...
        /// <param name="empId">员工ID</param>
        /// <param name="companyId">所属公司ID</param>
        /// <param name="isContainDeleted">包含已删除员工的数据</param>
        /// <returns></returns>
        public static OrgM_Dept GetEmpMainDept(Guid empId, Guid? companyId = null, bool isContainDeleted = false)
        {
            OrgM_EmpDeptDuty empPosition = GetEmpMainPosition(empId, companyId, isContainDeleted);
        ...
        /// <param name="empId">员工ID</param>
        /// <param name="companyId">所属公司ID</param>
        /// <param name="isContainDeleted">是否包含已删除员工数据</param>
        /// <returns></returns>
        public static string GetEmpMainDeptName(Guid empId, Guid? companyId = null, bool isContainDeleted = false)
        {
            OrgM_Dept dept = GetEmpMainDept(empId, companyId, isContainDeleted);
            if (dept != null)
        ...
        /// <param name="companyId">所属公司ID</param>
        /// <returns></returns>
        public static List<Guid> GetEmpChildsEmpIds(Guid empId, bool isDirect = true, Guid? deptId = null, Guid? companyId = null)
        {
            List<OrgM_Emp> emps = GetEmpChildsEmps(empId, isDirect, deptId, companyId).Where(x => x != null).ToList();
            return emps.Select(x => x.Id).ToList();
        }
            ...
            if (!string.IsNullOrEmpty(username))
            {
                switch (GlobalSet.EmpUserNameConfigRule)
                {
                    ...
                    case UserNameAndEmpConfigRule.EmailPre:
                        emp = GetAllEmps(x => x.Email != null && x.Email.Contains("@"), true).Where(x => x.Email.Substring(0, x.Email.IndexOf("@")).ToLower() == username.ToLower()).FirstOrDefault();
                        break;
                    case UserNameAndEmpConfigRule.Email:
                        emp = GetAllEmps(null, true).Where(x => x.Email != null && x.Email.ToLower() == username.ToLower()).FirstOrDefault();
                        break;
                    case UserNameAndEmpConfigRule.Mobile:
                        emp = GetAllEmps(x => x.Mobile != null && x.Mobile == username, true).FirstOrDefault();
                        break;
                }
            }
            return emp;
