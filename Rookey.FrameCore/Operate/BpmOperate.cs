...
namespace Rookey.Frame.Operate.Base
{
    ...
    public static class BpmOperate
    {
        ...
        /// <returns></returns>
        public static Bpm_WorkToDoList GetWorkTodo(Guid todoId)
        {
            string errMsg = string.Empty;
            Bpm_WorkToDoList todo = CommonOperate.GetEntityById<Bpm_WorkToDoList>(todoId, out errMsg);
            if (todo != null) return todo;
            Bpm_WorkToDoListHistory todoHistory = CommonOperate.GetEntityById<Bpm_WorkToDoListHistory>(todoId, out errMsg);
            if (todoHistory != null)
            {
        ...
        /// <param name="todoId">待办ID</param>
        /// <returns></returns>
        public static Guid GetWorkflowIdByTodoId(Guid todoId)
        {
            Bpm_WorkToDoList todo = GetWorkTodo(todoId);
            if (todo != null)
            {
                if (!todo.Bpm_WorkFlowInstanceId.HasValue || todo.Bpm_WorkFlowInstanceId.Value == Guid.Empty)
                    return Guid.Empty;
                Bpm_WorkFlowInstance flowInst = BpmOperate.GetWorkflowInstanceById(todo.Bpm_WorkFlowInstanceId.Value);
                if (flowInst != null)
                {
                    if (flowInst.Bpm_WorkFlowId.HasValue)
                        return flowInst.Bpm_WorkFlowId.Value;
                    else
                        return Guid.Empty;
                }
                string errMsg = string.Empty;
                Bpm_WorkFlowInstanceHistory workflowInstHistory = CommonOperate.GetEntity<Bpm_WorkFlowInstanceHistory>(x => x.RecordId == todo.Bpm_WorkFlowInstanceId.Value, null, out errMsg);
                if (workflowInstHistory != null)
                {
                    if (workflowInstHistory.Bpm_WorkFlowId.HasValue)
                        return workflowInstHistory.Bpm_WorkFlowId.Value;
                }
            }
            return Guid.Empty;
        }
        ...
        /// <returns></returns>
        public static string GetWorkNodeNameById(Guid workNodeId)
        {
            Bpm_WorkNode node = GetWorkNode(workNodeId);
            if (node != null)
                return node.Name;
            return string.Empty;
        }
        ...
        /// <returns></returns>
        public static Guid GetWorkNodeIdByName(string nodeName)
        {
            if (string.IsNullOrEmpty(nodeName))
                return Guid.Empty;
            Bpm_WorkNode node = GetAllWorkNodes(x => x.Name == nodeName).FirstOrDefault();
            if (node != null)
                return node.Id;
            return Guid.Empty;
        }
        ...
        public static List<Bpm_WorkNode> GetNextWorkNodes(Guid workflowId, Guid currNodeId)
        {
            ...
            if (workLines != null && workLines.Count > 0)
            {
                ...
                if (workNodes == null) workNodes = new List<Bpm_WorkNode>();
                int endNodeType = (int)WorkNodeTypeEnum.End;
                if (workNodes.Count > 1 && workLines.Count > 1 && workNodes.Select(x => x.WorkNodeType).Contains(endNodeType))
                {
                    //下一节点有多个并且包含结束节点时
                    return workNodes;
                }
        ...
        /// <param name="workFlowInst">流程实例</param>
        /// <param name="empId">被代理人</param>
        /// <returns></returns>
        public static Guid GetFlowProxyEmpId(Guid workflowId, Guid workNodeId, Bpm_WorkFlowInstance workFlowInst, Guid empId)
        {
            if (workFlowInst == null || !workFlowInst.OrgM_EmpId.HasValue)
                return empId;
            string errMsg = string.Empty;
            #region 定义找代理人函数
            //定义找代理人函数
            Func<Guid, List<Bpm_FlowProxy>, Guid> action = (Guid startEmpId, List<Bpm_FlowProxy> flowProxyList) =>
            {
                if (flowProxyList == null || flowProxyList.Count == 0)
                    return Guid.Empty;
                //找有效期内的代理
                flowProxyList = flowProxyList.Where(x => (x.ValidStartTime == null && x.ValidEndTime == null) || (x.ValidStartTime != null && x.ValidEndTime == null && x.ValidStartTime.Value < DateTime.Now) || (x.ValidEndTime != null && x.ValidStartTime == null && x.ValidEndTime.Value > DateTime.Now) || (x.ValidStartTime != null && x.ValidEndTime != null && x.ValidStartTime.Value < DateTime.Now && x.ValidEndTime.Value > DateTime.Now)).ToList();
                if (flowProxyList.Count == 0) //都已失效
                    return Guid.Empty;
                if (flowProxyList.Where(x => x.OrgM_DeptId.HasValue && x.OrgM_DeptId.Value != Guid.Empty).Count() > 0)
                {
                    OrgM_Dept dept = OrgMOperate.GetEmpMainDept(startEmpId);
                    if (dept != null)
                    {
                        //找发起人当前部门代理人
                        Bpm_FlowProxy flowProx = flowProxyList.Where(x => x.OrgM_DeptId == dept.Id).FirstOrDefault();
                        if (flowProx != null && flowProx.OrgM_EmpProxyId.HasValue && flowProx.OrgM_EmpProxyId.Value != Guid.Empty)
                        {
                            return flowProx.OrgM_EmpProxyId.Value;
                        }
                        //找发起人上级部门代理人
                        if (dept.ParentId.HasValue && dept.ParentId.Value != Guid.Empty)
                        {
                            flowProx = flowProxyList.Where(x => x.OrgM_DeptId == dept.ParentId.Value).FirstOrDefault();
                            if (flowProx != null && flowProx.OrgM_EmpProxyId.HasValue && flowProx.OrgM_EmpProxyId.Value != Guid.Empty)
                            {
                                return flowProx.OrgM_EmpProxyId.Value;
                            }
                            ...
                            {
                                flowProx = flowProxyList.Where(x => x.OrgM_DeptId == parentDept.ParentId.Value).FirstOrDefault();
                                if (flowProx != null && flowProx.OrgM_EmpProxyId.HasValue && flowProx.OrgM_EmpProxyId.Value != Guid.Empty)
                                {
                                    return flowProx.OrgM_EmpProxyId.Value;
                                }
                                //找发起人上上上级部门代理人
                                OrgM_Dept parentParentDept = OrgMOperate.GetDeptById(parentDept.ParentId.Value);
                                if (parentParentDept != null && parentParentDept.ParentId.HasValue && parentParentDept.ParentId.Value != Guid.Empty)
                                {
                                    flowProx = flowProxyList.Where(x => x.OrgM_DeptId == parentParentDept.ParentId.Value).FirstOrDefault();
                                    if (flowProx != null && flowProx.OrgM_EmpProxyId.HasValue && flowProx.OrgM_EmpProxyId.Value != Guid.Empty)
                                    {
                                        return flowProx.OrgM_EmpProxyId.Value;
                                    }
                ...
                {
                    Guid? proxyEmpId = flowProxyList.FirstOrDefault().OrgM_EmpProxyId;
                    if (proxyEmpId.HasValue && proxyEmpId.Value != Guid.Empty)
                    {
                        return proxyEmpId.Value;
                    }
                }
                return Guid.Empty;
            };
            #endregion
            Guid starter = workFlowInst.OrgM_EmpId.Value;
            List<Bpm_FlowProxy> flowProxys = CommonOperate.GetEntities<Bpm_FlowProxy>(out errMsg, x => x.Bpm_WorkFlowId == workflowId && x.Bpm_WorkNodeId == workNodeId && x.OrgM_EmpId == empId && x.IsValid == true, null, false);
            //流程+节点被代理
            Guid tempProxyEmpId = action(starter, flowProxys);
            if (tempProxyEmpId != Guid.Empty)
                return tempProxyEmpId;
            //整支流程被代理
            flowProxys = CommonOperate.GetEntities<Bpm_FlowProxy>(out errMsg, x => x.Bpm_WorkFlowId == workflowId && (x.Bpm_WorkNodeId == null || x.Bpm_WorkNodeId == Guid.Empty) && x.OrgM_EmpId == empId && x.IsValid == true, null, false);
            tempProxyEmpId = action(starter, flowProxys);
            if (tempProxyEmpId != Guid.Empty)
                return tempProxyEmpId;
            //所有流程被代理
            flowProxys = CommonOperate.GetEntities<Bpm_FlowProxy>(out errMsg, x => (x.Bpm_WorkFlowId == null || x.Bpm_WorkFlowId == Guid.Empty) && (x.Bpm_WorkNodeId == null || x.Bpm_WorkNodeId == Guid.Empty) && x.OrgM_EmpId == empId && x.IsValid == true, null, false);
            tempProxyEmpId = action(starter, flowProxys);
            if (tempProxyEmpId != Guid.Empty)
                return tempProxyEmpId;
            return empId;
        ...
        /// <param name="workFlowInst">流程实例</param>
        /// <param name="principalDic">被代理人</param>
        /// <param name="cErrMsg">找不到处理人时自定义提示信息</param>
        /// <returns>返回员工ID集合</returns>
        public static List<Guid> GetNextNodeHandler(Bpm_WorkNode nextNode, UserInfo currUser, Bpm_WorkFlowInstance workFlowInst, out Dictionary<Guid, Guid?> principalDic, out string cErrMsg)
        {
            cErrMsg = string.Empty;
            principalDic = new Dictionary<Guid, Guid?>();
            ...
            //调用自定义查找结点处理人方法
            object[] args = new object[] { workFlowInst.RecordId, nextNode.Name, workFlowInst.Id, currUser, cErrMsg };
            object rsObj = CommonOperate.ExecuteCustomeOperateHandleMethod(moduleId, "GetNodeHandler", args);
            cErrMsg = args[args.Length - 1].ObjToStr();
            if (rsObj != null)
            {
                try
                {
            ...
            if (empIds.Count == 0)
            {
                ...
                switch (nextNode.HandlerTypeOfEnum) //处理者类型
                {
                    ...
                    case NodeHandlerTypeEnum.FormFieldValue: //表单字段值
                        {
                            if (!string.IsNullOrEmpty(nextNode.FormFieldName) && workFlowInst.RecordId != Guid.Empty)
                            {
                                string tempMsg = string.Empty;
                                object model = CommonOperate.GetEntityById(moduleId, workFlowInst.RecordId, out tempMsg);
                                if (model != null)
                                {
                                    string fvs = CommonOperate.GetModelFieldValueByModel(moduleId, model, nextNode.FormFieldName).ObjToStr();
                                    if (!string.IsNullOrEmpty(fvs))
                                    {
                                        string[] token = fvs.Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                                        foreach (string str in token)
                                        {
                                            Guid tempEmpId = str.ObjToGuid();
                    ...
                    case NodeHandlerTypeEnum.StarterLevel3DeptLeader: //发起者第三层级部门负责人
                        ...
                        break;
                    case NodeHandlerTypeEnum.StarterLevel4DeptLeader: //发起者第四层级部门负责人
                        {
                            if (workFlowInst.OrgM_EmpId.HasValue && workFlowInst.OrgM_EmpId.Value != Guid.Empty)
                            {
                                OrgM_Dept dept = OrgMOperate.GetEmpMainDept(workFlowInst.OrgM_EmpId.Value, null, true);
                    ...
                    case NodeHandlerTypeEnum.LastHandlerLevel3DeptLeader: //上一处理者第三层级部门负责人
                        ...
                        break;
                    case NodeHandlerTypeEnum.LastHandlerLevel4DeptLeader: //上一处理者第四层级部门负责人
                        {
                            if (currUser.EmpId.HasValue && currUser.EmpId.Value != Guid.Empty)
                            {
                                OrgM_Dept dept = OrgMOperate.GetEmpMainDept(currUser.EmpId.Value, null, true);
            ...
            {
                Guid proxyEmpId = GetFlowProxyEmpId(workflowId, nextNode.Id, workFlowInst, empId);
                principalDic.Add(proxyEmpId, proxyEmpId != empId ? empId : (Guid?)null);
        ...
        public static List<Guid> GetLaunchNodeHandler(Guid moduleId)
        {
            ...
            switch (launchNode.HandlerTypeOfEnum) //处理者类型
            {
                case NodeHandlerTypeEnum.All:
                    {
                        empIds = OrgMOperate.GetAllEmps(null, true).Select(x => x.Id).ToList();
                    }
                    break;
        ...
        public static List<Bpm_WorkNode> GetBackNodes(Guid toDoTaskId)
        {
            ...
            if (workNode != null && workNode.Bpm_WorkFlowId.HasValue && workNode.Bpm_WorkFlowId.Value != Guid.Empty)
            {
                if (workNode.BackTypeOfEnum == NodeBackTypeEnum.BackToLast) //只能回退到上一结点
                {
                    ...
                    string whereSql = string.Format("Bpm_WorkNodeId IN('{0}') AND Id IN(SELECT Bpm_WorkNodeInstanceId FROM dbo.Bpm_WorkToDoList WHERE WorkAction IN(2,3) AND Bpm_WorkFlowInstanceId IN(SELECT Bpm_WorkFlowInstanceId FROM dbo.Bpm_WorkToDoList WHERE Id='{1}')) ORDER BY AutoIncrmId DESC", preNodeIdStr, toDoTaskId.ToString());
        ...
        public static List<FormButton> GetWorkNodeFormBtns(Guid workflowId, Guid workNodeId)
        {
            ...
            if (nodeConfigBtns.Count > 0)
            {
                ...
                foreach (Bpm_NodeBtnConfig btnConfig in nodeConfigBtns)
                {
                    ...
                    string icon = flowBtn.ButtonIcon;
                    ...
                    btns.Add(new FormButton() { TagId = string.Format("flowBtn_{0}", flowBtn.Id), IconType = iconType, Icon = icon, DisplayText = btnConfig.BtnDisplay, ClickMethod = clickMethod, Sort = sort });
                    sort++;
                }
            }
            else
            {
                ...
                if (flowBtn != null)
                {
                    ...
                    if (string.IsNullOrEmpty(icon))
                        icon = "eu-icon-approvalok";
                    btns.Add(new FormButton() { TagId = string.Format("flowBtn_{0}", flowBtn.Id), IconType = ButtonIconType.FlowAgree, Icon = icon, DisplayText = flowBtn.ButtonText, ClickMethod = "ApprovalFlow(this)", Sort = 1 });
                }
            }
            return btns;
        }
        ...
        public static string StartProcess(Guid moduleId, Guid recordId, UserInfo currUser, Guid? parentFlowInstId = null, Guid? parentWorkTodoId = null, List<Guid> detailIds = null)
        {
            ...
            return StartProcessByFlowId(workFlow.Id, recordId, currUser, parentFlowInstId, parentWorkTodoId, detailIds);
        }
        ...
        /// <param name="detailIds">当前子流程需要发起的所有明细ID集合，判断所有子流程是否结束时用到</param>
        /// <returns></returns>
        public static string StartProcessByFlowId(Guid workflowId, Guid recordId, UserInfo currUser, Guid? parentFlowInstId = null, Guid? parentWorkTodoId = null, List<Guid> detailIds = null)
        {
            string errMsg = string.Empty;
            string errPrex = GetFlowErrPrex(WorkActionEnum.Starting);
            Bpm_WorkFlow workFlow = GetWorkflow(workflowId);
        ...
        /// <param name="returnNode">针对回退时回退节点信息</param>
        /// <param name="directHandler">针对指派时被指派人ID</param>
        /// <returns></returns>
        public static string ApproveProcess(Guid workTodoId, string approvalOpinions, UserInfo currUser, WorkActionEnum workAction, ReturnNodeInfo returnNode = null, Guid? directHandler = null)
        {
            ...
            if (workAction == WorkActionEnum.Returning) //回退时
            {
                if (returnNode == null || returnNode.ReturnNode == null)
                    returnNode = new ReturnNodeInfo() { ReturnNode = GetPrexNode(workFlow.Id, workNode.Id).FirstOrDefault() };
                if (returnNode == null || returnNode.ReturnNode == null)
        ...
        private static string HandleProcess(Bpm_WorkFlow workflow, Bpm_WorkFlowInstance workFlowInst, Bpm_WorkNode workNode, Bpm_WorkNodeInstance workNodeInst, UserInfo currUser, WorkActionEnum workAction, string approvalOpinions, Bpm_WorkToDoList workTodo, ReturnNodeInfo returnNode = null, Guid? directHandler = null, Guid? parentWorkTodoId = null, List<Guid> detailIds = null)
        {
            ...
            if (workAction == WorkActionEnum.Returning && (returnNode == null || returnNode.ReturnNode == null))
                return string.Format("{0}回退节点不存在，请联系系统运维人员", errPrex);
            ...
            string nextNodeName = string.Empty; //下一审批节点
            #region 自动跳转相关
            ...
            #endregion
            #region 子流程预处理
            bool displayParentFlow = true; //是否显示父流程下一节点待办
            ...
            if (workFlowInst.ParentId.HasValue && workFlowInst.ParentId.Value != Guid.Empty && isChildFlowHandle)
            {
                ...
                if (parentNode != null && parentNode.Bpm_WorkFlowId.HasValue)
                {
                    ...
                    {
                        if (parentNode.Bpm_WorkFlowId.HasValue && parentNode.Bpm_WorkFlowId.Value != Guid.Empty)
                        {
                            int parentNodeNum = GetWorkNodesOfFlow(parentNode.Bpm_WorkFlowId.Value).Where(x => x.WorkNodeType != 0 && x.WorkNodeType != 1).Count();
                            if (parentNodeNum == 2 && parentNextNode.HandlerTypeOfEnum == NodeHandlerTypeEnum.Starter)
                                displayParentFlow = false;
                        }
            ...
            {
                Bpm_WorkNodeInstance nodeInst = CommonOperate.GetEntity<Bpm_WorkNodeInstance>(x => x.Bpm_WorkFlowInstanceId == workFlowInst.Id && x.Bpm_WorkNodeId == returnNode.ReturnNode.Id, null, out errMsg, new List<string>() { "Id" });
                if (nodeInst != null)
                {
                    ...
                    {
                        List<Guid> tempReturnHandlers = tempTodos.Where(x => x.OrgM_EmpId.HasValue).Select(x => x.OrgM_EmpId.Value).ToList();
                        if (tempReturnHandlers.Count > 0)
                            returnHandles.Add(tempReturnHandlers.FirstOrDefault());
                    }
                }
            }
            #endregion
            ...
            lock (lockObj)
            {
                ...
                CommonOperate.TransactionHandle((conn) =>
                {
                    ...
                    if (workFlowInst.Id == Guid.Empty || workTodo == null)
                    {
                        ...
                        {
                            errMsg = string.Format("{0}流程实例初始化失败，{1}", errPrex, errMsg);
                    ...
                    if (!isParallelApproval) //非并行审批
                    {
                        ...
                        {
                            Dictionary<Guid, Guid?> principalDic = new Dictionary<Guid, Guid?>();
                            #region 找下一审批结点及处理人
                            //处理当前结点实例
                            ...
                            {
                                errMsg = string.Format("{0}流程结点实例状态更新失败，{1}", errPrex, errMsg);
                                throw new Exception(errMsg);
                            }
                            //待办处理
                            ...
                            {
                                Bpm_WorkNode returnBackNextNode = null;
                                if (workAction == WorkActionEnum.Approving && workTodo != null && workTodo.IsReturnBack == true &&
                                    workTodo.ReturnBackNextNode.HasValue && workTodo.ReturnBackNextNode.Value != Guid.Empty)
                                {
                                    returnBackNextNode = GetWorkNode(workTodo.ReturnBackNextNode.Value); //回退审批后下一处理节点
                                }
                                if (returnBackNextNode == null) //非回退时
                                {
                                    ...
                                    int endNodeType = (int)WorkNodeTypeEnum.End;
                                    if (nextNodes.Count > 1 && nextNodes.Select(x => x.WorkNodeType).Contains(endNodeType))
                                    {
                                        //下一节点有多个并且包含结束节点时
                                        Bpm_WorkNode tempNextNode = GetNextActionNode(nextNodes, workflow, workNode, workFlowInst, workNodeInst);
                                        if (tempNextNode != null && tempNextNode.WorkNodeTypeOfEnum == WorkNodeTypeEnum.End) //下一结点为结束结点
                                            nextNodes = new List<Bpm_WorkNode>();
                                    }
                                }
                                else
                                {
                                    nextNodes = new List<Bpm_WorkNode>() { returnBackNextNode };
                                }
                            }
                            else if (workAction == WorkActionEnum.Returning) //回退
                            {
                                nextNodes = new List<Bpm_WorkNode>() { returnNode.ReturnNode };
                            ...
                            string cErrMsg = string.Empty; //找不到处理人时自定义提示信息
                            if (nextNodes.Count > 1) //对应的下一结点有多个
                            {
                                ...
                                handleEmpIds = GetNextNodeHandler(nextApprovalNode, currUser, workFlowInst, out principalDic, out cErrMsg);
                            }
                            ...
                            {
                                string tempMsg = string.IsNullOrEmpty(cErrMsg) ? string.Format("{0}找不到节点【{1}】处理人", errPrex, nextApprovalNode.Name) : cErrMsg;
                            ...
                            if (result == Guid.Empty)
                            {
                                errMsg = string.Format("{0}流程实例状态更新失败，{1}", errPrex, errMsg);
                                throw new Exception(errMsg);
                            }
                            #endregion
                            #region 处理当前待办
                            ...
                            if (workTodo == null) //发起节点
                            {
                                ...
                                if (result == Guid.Empty)
                                {
                                    errMsg = string.Format("{0}当前待办任务初始化失败,{1}", errPrex, errMsg);
                                    throw new Exception(errMsg);
                                }
                                #endregion
                            }
                            else //审批节点
                            {
                                ...
                                if (result == Guid.Empty)
                                {
                                    errMsg = string.Format("{0}当前待办任务状态更新失败,{1}", errPrex, errMsg);
                                    throw new Exception(errMsg);
                                }
                                #endregion
                            }
                            ...
                            if (nextApprovalNode != null)
                            {
                                nextNodeName = nextApprovalNode.Name;
                                #region 自动跳转处理
                                ...
                                if (nextApprovalNode.HandleStrategyOfEnum == HandleStrategyTypeEnum.AllAgree) //下一结点为并行结点
                                {
                                    ...
                                    foreach (Guid tempEmpId in handleEmpIds)
                                    {
                                        ...
                                        {
                                            errMsg = string.Format("{0}下一审批结点实例初始化失败,{1}", errPrex, errMsg);
                                            throw new Exception(errMsg);
                                        }
                                ...
                                foreach (Guid empId in handleEmpIds)
                                {
                                    ...
                                    Bpm_WorkToDoList toDo = new Bpm_WorkToDoList()
                                    {
                                        ...
                                        PrincipalId = principalDic != null && principalDic.ContainsKey(empId) ? principalDic[empId] : (Guid?)null
                                    };
                                ...
                                if (workAction == WorkActionEnum.Returning && nextTodoList.Count == 1 && returnNode.IsReturnBack.HasValue) //回退方式处理
                                {
                                    if (returnNode.IsReturnBack.Value)
                                    {
                                        nextTodoList.FirstOrDefault().IsReturnBack = true;
                                        if (returnNode.ReturnBackNextNode.HasValue && returnNode.ReturnBackNextNode.Value != Guid.Empty)
                                            nextTodoList.FirstOrDefault().ReturnBackNextNode = returnNode.ReturnBackNextNode.Value;
                                    }
                                }
                                ...
                                if (!rs)
                                {
                                    errMsg = string.Format("{0}下一审批节点待办任务初始化失败，{1}", errPrex, errMsg);
                                    throw new Exception(errMsg);
                                }
                                if (isUpdateNextHandler) //当前待办更新下一处理人
                                {
                                    ...
                                    if (result == Guid.Empty)
                                    {
                                        errMsg = string.Format("{0}当前待办任务下一处理人更新失败,{1}", errPrex, errMsg);
                                        throw new Exception(errMsg);
                                    }
                                }
                                ...
                                workflowStatus == WorkFlowStatusEnum.Obsoleted || isParallelHasRejectOver) //流程结束或拒绝或作废
                            {
                                ...
                                if (isChildFlowHandle) //当前为子流程
                                {
                                    if (parentNextNodeInstIds.Count > 0 && isAllChildFlowOver && displayParentFlow) //父流程下一审批节点存在并且所有子流程均结束
                                    {
                                        //显示父流程下一结点的所有待办
                        ...
                        else //流程指派
                        {
                            ...
                            if (result != Guid.Empty)
                            {
                                ...
                                if (result == Guid.Empty)
                                {
                                    errMsg = string.Format("{0}指派人待办任务初始化失败,{1}", errPrex, errMsg);
                                    throw new Exception(errMsg);
                                }
                            }
                            else
                ...
                if (string.IsNullOrEmpty(errMsg)) //流程处理成功
                {
                    ...
                    //执行流程操作完成事件
                    CommonOperate.ExecuteCustomeOperateHandleMethod(workflow.Sys_ModuleId.Value, "AfterFlowOperateCompleted", new object[] { workFlowInst.RecordId, workNode.Name, currUser, string.IsNullOrEmpty(errMsg), workAction, workflowStatus, approvalOpinions, nextNodeName });
                    //事件通知
                    ...
                    if (isChildFlowHandle) //当前为子流程
                    {
                        ...
                        foreach (Guid empId in tempNextHandlers) //对下一处理人待办处理包含当前待办人待办
                        {
                            ...
                            var childTodoGroups = childTodos.GroupBy(x => x.Bpm_WorkNodeId).ToList();
                            foreach (var group in childTodoGroups)
                            {
                                var tempList = group.OrderBy(x => x.AutoIncrmId);
                                if (tempList.Where(x => x.IsDeleted == false).Count() == 0) //当前没有已经显示的子流程待办
                                {
                                    Bpm_WorkToDoList tempTodo = tempList.FirstOrDefault();
                    ...
                    if (isStartSubFlow) //需要发子流程或分支流程
                    {
                        ...
                        string tableName = subModelType.Name; //ModelConfigHelper.GetModuleTableName(subModelType);
                        //获取当前记录的明细ID集合
                        ...
                        string sql = string.Format("SELECT Id FROM {0} WHERE {1}Id='{2}' AND IsDeleted=0", tableName, mainModelType.Name, workTodo.RecordId.ToString());
                        ...
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            ...
                            List<Guid> startedSubIds = new List<Guid>(); //已发起的子流程ID集合
                            foreach (Guid subId in subIds)
                            {
                                ...
                                {
                                    string subFlowType = workNode.SubFlowTypeOfEnum == SubFlowTypeEnum.ChildFlow ? "子流程" : "分支流程";
                                    //记录日志
                                    LogOperate.AddOperateLog(currUser, "待办任务", string.Format("{0}发起失败", subFlowType), string.Format("{0}发起失败，流程ID【{1}】，父流程ID【{2}】", string.Format("{0}发起失败", tempParentTodoId.HasValue ? "子流程" : "分支流程"), subId, workTodo.Id), false, msg);
                                    //清理流程数据以便下次能重新发起
                                    string tempErrMsg = string.Empty;
                                    startedSubIds.Add(workFlowInst.RecordId); //添加主流程记录ID
                                    StringBuilder sqlSb = new StringBuilder();
                                    foreach (Guid tempSubId in startedSubIds)
                                    {
                                        sqlSb.AppendLine(string.Format("DELETE FROM dbo.Bpm_WorkNodeInstance WHERE Bpm_WorkFlowInstanceId IN(SELECT Id FROM dbo.Bpm_WorkFlowInstance WHERE RecordId='{0}');", tempSubId));
                                        sqlSb.AppendLine(string.Format("DELETE FROM dbo.Bpm_WorkNodeInstance WHERE Bpm_WorkFlowInstanceId IN(SELECT Id FROM dbo.Bpm_WorkFlowInstanceHistory WHERE RecordId='{0}');", tempSubId));
                                        sqlSb.AppendLine(string.Format("DELETE FROM dbo.Bpm_WorkNodeInstanceHistory WHERE Bpm_WorkFlowInstanceId IN(SELECT Id FROM dbo.Bpm_WorkFlowInstance WHERE RecordId='{0}');", tempSubId));
                                        sqlSb.AppendLine(string.Format("DELETE FROM dbo.Bpm_WorkNodeInstanceHistory WHERE Bpm_WorkFlowInstanceId IN(SELECT Id FROM dbo.Bpm_WorkFlowInstanceHistory WHERE RecordId='{0}');", tempSubId));
                                        sqlSb.AppendLine(string.Format("DELETE FROM dbo.Bpm_WorkToDoList WHERE RecordId='{0}';", tempSubId));
                                        sqlSb.AppendLine(string.Format("DELETE FROM dbo.Bpm_WorkToDoListHistory WHERE RecordId='{0}';", tempSubId));
                                        sqlSb.AppendLine(string.Format("DELETE FROM dbo.Bpm_WorkFlowInstance WHERE RecordId='{0}';", tempSubId));
                                        sqlSb.AppendLine(string.Format("DELETE FROM dbo.Bpm_WorkFlowInstanceHistory WHERE RecordId='{0}';", tempSubId));
                                    }
                                    //CommonOperate.ExecuteNonQuery(out tempErrMsg, sqlSb.ToString(), null, connStr, dbType);
                                    //子流程发起失败后直接返回格式化的错误信息
                                    Bpm_WorkFlow subWorkFlow = GetWorkflow(workNode.Bpm_WorkFlowSubId.Value);
                                    string subWorkflowName = string.IsNullOrEmpty(subWorkFlow.DisplayName) ? subWorkFlow.Name : subWorkFlow.DisplayName;
                                    return string.Format("{0}【{1}】发起失败，异常信息：{2}", subFlowType, subWorkflowName, msg);
                                }
                                else
                                {
                                    startedSubIds.Add(subId); //装载已发起的子流程记录ID
                                }
                            }
                        }
                    }
                    ...
                    else if ((autoJumpNode != null && autoJumpNodeInst != null) || (isChildFlowHandle && parentNextNodeInstIds.Count > 0 && isAllChildFlowOver))
                    {
                        ...
                        if (isChildFlowHandle && parentNextNodeInstIds.Count > 0 && isAllChildFlowOver) //当前为子流程并且所有其他子流程均结束
                        {
                                ...
                                workflowStatus == WorkFlowStatusEnum.Obsoleted) //当前子流程已结束
                            {
                                ...
                                if (tempNextNodes.Count == 0 && (workflowStatus == WorkFlowStatusEnum.Refused || workflowStatus == WorkFlowStatusEnum.Obsoleted)) //父流程当前自动审批结点为最后一个结点时，判断子流程只要有一个子流程审批通过则该父流程结点为同意，否则为拒绝
                                {
        ...
        /// <param name="currUser">当前用户</param>
        public static void TransferWorkToDoHistory(Bpm_WorkFlowInstance workFlowInst, UserInfo currUser)
        {
        ...
        public static void AsyncUpdateRecordFlowStatus(Guid moduleId, Guid id, WorkFlowStatusEnum status, UserInfo currUser = null)
        {
            ...
            if (rs) //处理成功后
            {
                ...
                Task.Factory.StartNew(() =>
                {
                    //明细未启用流程时，将主模块流程状态更新到明细模块单据中
                    string foreignField = string.Format("{0}Id", SystemOperate.GetModuleTableNameById(moduleId));
                    List<Sys_Module> detailModules = SystemOperate.GetDetailModules(moduleId);
                    foreach (Sys_Module detailModule in detailModules)
                    {
                        if (BpmOperate.IsEnabledWorkflow(detailModule.Id))
                            continue;
                        Type detailModelType = CommonOperate.GetModelType(detailModule.TableName);
                        int tempStatus = (int)status;
                        if (ModelConfigHelper.IsModelEnableCache(detailModelType)) //明细模块启用缓存
                        {
                            List<ConditionItem> items = new List<ConditionItem>() { new ConditionItem() { Field = foreignField, Method = QueryMethod.Equal, Value = id } };
                            object exp = CommonOperate.GetQueryCondition(detailModule.Id, items);
                            CommonOperate.UpdateRecordsByExpression(detailModule.Id, new { FlowStatus = tempStatus }, exp, out errMsg);
                        }
                        else //未启用缓存
                        {
                            string sql = string.Format("UPDATE {0} SET FlowStatus={1} WHERE {2}='{3}'", detailModelType.Name, tempStatus, foreignField, id);
                            DatabaseType dbtype = DatabaseType.MsSqlServer;
                            string connStr = ModelConfigHelper.GetModelConnStr(detailModelType, out dbtype, false);
                            CommonOperate.ExecuteNonQuery(out errMsg, sql, null, connStr, dbtype);
                        }
                    }
                });
            }
        ...
        public static List<ApprovalInfo> GetRecordApprovalInfos(Bpm_WorkFlowInstance workFlowInst, bool onlyGetLastOne = false)
        {
            ...
            if (todoList == null || todoList.Count == 0) //从当前待办中取
            {
                List<bool> descs = onlyGetLastOne ? new List<bool>() { true } : new List<bool>() { false };
                todoList = CommonOperate.GetEntities<Bpm_WorkToDoList>(out errMsg, x => x.Bpm_WorkFlowInstanceId == workFlowInst.Id, null, false, new List<string>() { "FinishDate" }, descs);
            }
            if (todoList != null && todoList.Count > 0)
            {
                Dictionary<Guid, List<Guid>> tempDic = new Dictionary<Guid, List<Guid>>(); //针对并行审批时保留前一处理人集合
                foreach (Bpm_WorkToDoList todo in todoList)
                {
                    ...
                    if (todo.IsInitHandler)
                    {
                        ...
                        {
                            empIdStr = OrgMOperate.GetEmpNameContainsQuit(todo.OrgM_EmpId.Value);
                        }
                    ...
                    if (!string.IsNullOrEmpty(todo.NextNodeHandler))
                    {
                        ...
                        if (currNode.HandleStrategyOfEnum == HandleStrategyTypeEnum.AllAgree)
                        {
                            if (tempDic.ContainsKey(currNode.Id))
                                tempDic[currNode.Id].Add(todo.OrgM_EmpId.Value);
                            else
                                tempDic.Add(currNode.Id, new List<Guid>() { todo.OrgM_EmpId.Value });
                            if (tempDic[currNode.Id].Count < tempEmpIds.Count)
                                tempEmpIds = tempEmpIds.Where(x => !tempDic[currNode.Id].Contains(x)).ToList();
                        }
                        if (tempEmpIds.Count > 0)
                        {
                            foreach (Guid tempEmpId in tempEmpIds)
                            {
                                ...
                                if (tempEmpId != Guid.Empty)
                                {
                                    empName = OrgMOperate.GetEmpNameContainsQuit(tempEmpId);
                                    if (currFlow.OrgM_EmpId.HasValue && currFlow.OrgM_EmpId.Value == tempEmpId)
                                    {
                                        empName += "(找不到处理人转由流程管理人处理)";
                                    }
                                    else if (todo.Bpm_WorkNodeId.HasValue && todo.Bpm_WorkNodeId.Value != Guid.Empty)
                                    {
                                        Bpm_WorkToDoList tempTodo = todoList.Where(x => x.OrgM_EmpId == tempEmpId).FirstOrDefault();
                                        if (tempTodo != null && tempTodo.PrincipalId.HasValue && tempTodo.PrincipalId.Value != tempEmpId)
                                        {
                                            empName += "（代理）";
                                        }
                                    }
