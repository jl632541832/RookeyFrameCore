...
function ForeignKeyFormatter(value, row, index, moduleName, fieldName, foreignModuleName, paramsObj, otherForeignParams, gridId) {
    ...
    if (page == 'fdGrid' || page == 'otGrid' || page == 'edit') return v;
    var title = v;
    if (!IsGuid(v) && otherForeignParams && otherForeignParams.length > 0) {
...
function DicFieldFormatter(value, row, index, moduleName, fieldName, dicData, paramsObj) {
    ...
    if ((value || value == 0) && dicData && dicData.length > 0) {
        var dic = null;
        try {
            dic = eval("(" + decodeURIComponent(dicData) + ")");
        } catch (ex) { }
        if (dic && dic.length > 0) {
            var flag = true; //未处理标识
            for (var i = 0; i < dic.length; i++) {
                ...
                if (arr.length > 1) {
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        for (var j = 0; j < dic.length; j++) {
                            var tempV = dic[j].Id;
                            if (arr[i] == tempV && tempV != "") {
                                if (textValues.length > 0)
                                    textValues += ',';
                                textValues += dic[j].Name;
                                break;
                            }
                        }
                    }
...
//moduleName:当前模块名称
//fieldName:当前字段名称
//valueFields:radio的值定义
//textFields:radio的显示名称定义
//paramsObj:字段参数对象
function RadioListFormatter(value, row, index, moduleName, fieldName, valueFields, textFields, paramsObj) {
    var v = value != undefined && value != null ? value : "";
    if (typeof (OverMutiCheckBoxFormatter) == 'function') {
        v = OverRadioListFormatter(value, row, index, moduleName, fieldName, valueFields, textFields, paramsObj);
        if (v != undefined && v != null)
            return v;
    }
    if (v.length > 0 && valueFields && textFields) {
        var token1 = valueFields.split(',');
        var token2 = textFields.split(',');
        if (token1.length == token2.length) {
            for (var i = 0; i < token1.length; i++) {
                if (token1[i] == v) {
                    v = token2[i];
...
//隐藏省略长度过长文本，采用事件触发方式查看详细内容
//value:字段值
//row:行对象
//index:行号 
//showType:展示方式（1-弹出方式）
function ShowHidedContextFormat(value, row, index, showType) {
    var viewType = 1;
    if (showType) viewType = showType;
    if (viewType == 1) {
        return '<a href="javascript:javascript:void(0)" title="' + value + '" class="easyui-tooltip"  onclick="ShowContext(this)">'
                + value
                + "</a> ";
    }
    else {
