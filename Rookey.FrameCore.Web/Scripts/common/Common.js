...
$(function () {
    ...
    try {
        $('.textarea-flag').next('span').addClass('textarea-span');
    } catch (e) { }
});
...
function SelectDialogData(obj, backFun, fieldAttr) {
    ...
    if (typeof (OverGetUrl) == "function") { //url重写方法
        url = OverGetUrl(fieldName, url);
    }
            ...
            var executeFun = function () {
                if (typeof (backFun) != "function") { //通用处理
                    ...
                    if (typeof (OnFieldSelect) == "function") { //触发字段选择事件
                        OnFieldSelect(rows, fieldName, valueField, textField, obj);
                    }
    ...
    var w = isTree ? 500 : 900;
    var h = isTree ? 490 : 500;
    var tempw = $(obj).attr("dw"); //自定义弹出框宽
    var temph = $(obj).attr("dh"); //自定义弹出框高
    if (tempw) {
...
//isMutiSelect:是否多选
//urlParams:额外的url参数
//btnSelectText:选择按钮text
function SelectModuleData(moduleIdOrName, backFun, condition, where, isMutiSelect, urlParams, btnSelectText) {
    if (!moduleIdOrName) {
    ...
    if (urlParams && urlParams.length > 0) {
        url += urlParams.substr(0, 1) == '&' ? urlParams : "&" + urlParams;
    }
    url += "&r=" + Math.random(); //弹出框页面url
    var toolbar = [{
        text: btnSelectText ? btnSelectText : "选择",
        iconCls: "eu-icon-ok",
        handler: function (e) {
            var win = topWin.GetCurrentDialogFrame()[0].contentWindow;
            ...
            if (typeof (backFun) == "function") { //有自定义函数时调用自定义函数
                var errMsg = backFun(data);
                if (errMsg && errMsg.length > 0) {
                    if (errMsg != ' ') {
                        topWin.ShowAlertMsg('提示', errMsg);
                    }
                    return;
                }
            }
    ...
    topWin.OpenDialog(title, url, toolbar, 900, 500, 'eu-icon-search');
}
...
//w:调整后宽
//h:调整后高
//setRegonId:需要调整regon宽度的regon domid
function ResizeGrid(gridId, w, h, setRegonId) {
    ...
    if (gridId && (width > 0 || height > 0)) {
        ...
        if (setRegonId && setRegonId.length > 0 && width > 0) {
            $('#' + setRegonId).width(width);
            $('#' + setRegonId).parent().width(width);
        }
    }
}

//隐藏mainRegion横向滚动条
function HiddenMainRegionScrollX() {
    $('#regon_main').css('overflow-x', 'hidden');
}
        ...
        if (field != null) {
            ...
            if (field.CanEdit) { //可编辑字段
                if (controlType == 0 || controlType == 12 || controlType == 15 || controlType == 100) { //文本框或文本域或密码框或标签框
                    ...
                    try {
                        if (key.substr(key.length - 2) == "Id" || key.substr(key.length - 4) != "Name") {
                            $("#" + key, form).attr('v', value).attr('value', value);
                        }
                        else {
                            var controlName = key.substr(0, 5) == "Other" ? key.substr(0, key.length - 4) : key.substr(0, key.length - 4) + "Id";
...
//isShowTip:是否显示提示
//isGet:是否get方式获取
function ExecuteCommonAjax(url, params, successBakFun, isShowTip, isGet, isNotSuccessTip) {
    var msgTitle = '操作提示';
...
function OnFieldLoadSuccess(fieldName, valueField, textField, isTree) {
    if (isTree) {
        ...
        //针对联动字段处理
        if (typeof (OverOnLinkFieldLoadSuccess) == "function") {
            OverOnLinkFieldLoadSuccess(fieldName, valueField, textField);
        }
    }
    //调用模块自定义事件
...
//textField:显示字段
//obj:当前控件
function OnFieldSelect(record, fieldName, valueField, textField, obj) {
    //调用模块自定义事件
    if (typeof (OverOnFieldSelect) == "function") {
        OverOnFieldSelect(record, fieldName, valueField, textField, obj);
    }
    //针对联动字段处理
    if (typeof (OverOnLinkFieldSelect) == "function") {
        OverOnLinkFieldSelect(record, fieldName, valueField, textField, obj);
    }
}
...
function ShowContext(obj) {
    ...
    var content = "<div style='margin:20px;word-wrap:break-word;word-break:break-all;'>" + title + "</div>";
    topWin.OpenDialog('查看详细信息', content, toolbar);
}
...
//username:用户名
//empId:员工ID
function SnsValid(validOkFun, des, username, empId) {
    if (empId == null || empId == undefined)
        empId = '';
    if (des == undefined || des == null)
        des = '';
    $.get('/User/GetCurrEmpMobile.html', { empId: empId, username: username }, function (result) {
        if (result && (result.M || result.Email)) {
            var mobile = result.M;
            var email = result.Email;
            if (username == undefined || username == null || username == '')
                username = result.Um;
            var content = "<table style='margin-top:6px;margin-left:12px;line-height:28px'><tr><td>验证方式：</td><td><label><input id='mobile' name='validtype' type='radio' value='" + mobile + "' checked='checked' onclick='SnsValidTypeChange(this)' />手机短信</label>&nbsp;&nbsp;<label><input id='email' name='validtype' type='radio' value='" + email + "' onclick='SnsValidTypeChange(this)' />邮箱</label></td></tr><tr><td><span id='spanvalid'>手机号</span>：</td><td><span id='mobileoremail'>" + mobile + "</span></td></tr><tr><td>请输入验证码：</td><td><input type='text' id='mvalcode' class='easyui-numberbox' style='width:100px' data-options='min:0,max:999999,precision:0'></input><a id='btnSendSns' um='" + username + "' mobile='" + mobile + "' des='" + des + "' style='margin-left: 3px;' class='easyui-linkbutton icon-white' onclick='SendSns(this)' data-options=\"iconCls:'eu-icon-email'\">发送验证码</a></td></tr><tr><td colspan='2' style='color:red'>如果发现手机号不正确，请联系HR员工专员更新手机号</td></tr></table>";
            var div = topWin.$("#page_dialog10");
            div.dialog({
                collapsible: false,
                minimizable: false,
                resizable: false,
                ...
                buttons: [{
                    ...
                    iconCls: "eu-icon-ok",
                    handler: function () {
                        var mvalcode = topWin.$("#mvalcode").val();
                        if (mvalcode == '') {
                            topWin.$.messager.alert("提示", '请输入验证码');
                            return;
                        }
                        var code = topWin.$('#btnSendSns').attr('code');
                        if (code == undefined || code == '') {
                            topWin.$.messager.alert("提示", '未发送验证码或验证码已过期，请重新发送');
                            return;
                        }
                        var md32 = hex_md5(mvalcode);
                        var md16 = md32.substr(8, 16);
                        if (md16 != code) { //验证通过
                            topWin.$.messager.alert("提示", '验证码错误');
                            return;
                        }
                        //更新验证码使用状态
                        $.get('/User/UpdateValidLog.html', { um: username, code: code });
                        div.dialog("close");
                        //调用自定义验证OK函数
                        if (typeof (validOkFun) == "function") {
                            validOkFun(div, username);
                        }
                    }
                }, {
                    text: '取消',
...
function SendSns(obj) {
    ...
    var timout = mobile.indexOf('@') > 0 ? 120 : 60; //按钮重发时间
    var n = timout;
    var it = setInterval(function () {
        $(obj).find('span.l-btn-text').text('(' + n + 's)');
        if (n <= 0) { //失效
            window.clearInterval(it);
            $(obj).linkbutton('enable');
            $(obj).find('span.l-btn-text').text('发送验证码');
        }
        n--;
    }, 1000);
    var timout2 = mobile.indexOf('@') > 0 ? 600 : 300;//验证码过期时间
    var rn = 0;
    ...
    var um = $(obj).attr('um');
    //发送短信验证码
    $.get('/User/SendSns.html', { m: mobile, um: um, des: des }, function (data) {
        if (data.Code) {
            $(obj).attr('code', data.Code);
        }
        topWin.$.messager.alert("提示", data.Msg);
    });
}

//验证类型变更事件
function SnsValidTypeChange(obj) {
    var domid = $(obj).attr('id');
    if (domid == 'mobile') {
        var mobile = $(obj).val();
        $('#spanvalid').text('手机号');
        $('#mobileoremail').text(mobile);
        $('#btnSendSns').attr('mobile', mobile);
    }
    else {
        var email = $(obj).val();
        $('#spanvalid').text('邮箱');
        $('#mobileoremail').text(email);
        $('#btnSendSns').attr('mobile', email);
    }
}

//回退选择节点后事件
//record:选择节点
function OnBackNodeSelect(record) {
    if (!record || !record.value) return;
    topWin.$('#nodeBackStyle').remove();
    if (topWin.$('#' + record.value).attr('dso') == '1') {
        topWin.$('#backNodes').parent().append('<div id="nodeBackStyle"><table style="line-height:20px;margin-top:10px;"><tr><td style="width:25px;"><input type="checkbox" id="isReturnBack" checked="checked"></td><td>是否直接返回当前节点<td></tr><tr><td colspan="2" style="color:red">提示：打勾表示退回人员重新处理流程后会跳过其他审批节点直接跳转至当前节点人审批，不打勾系统默认重新走流程。</td></tr></table></div>');
    }
}
