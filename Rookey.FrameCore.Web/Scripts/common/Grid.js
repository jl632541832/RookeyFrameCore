...
$(function () {
    ...
    setTimeout(function () {
        ...
        $("#searchform .easyui-textbox[foreignField='1'],#searchform .easyui-textbox[noforein='1']").each(function (i, domItem) {
            ...
            var tempMidOrMname = $(domItem).attr('moduleId') ? $(domItem).attr('moduleId') : moduleIdOrModuleName;
            FieldBindAutoCompleted(tempDom, tempMidOrMname, fieldName, displayTemplete, function (dom, item, fieldName, paramObj) {
                var text = item["f_Name"];
                var value = item["Id"];
    ...
    if ($('#div_complexSearch').attr('mutistyle') == '1') {
        var btnH = 38;
        if ($('#regon_main .gaidbtns').eq(0).find('a').length == 0) {
            btnH = 0;
            $('#regon_main .gaidbtns').eq(0).remove();
        }
        var trh = $('#div_complexSearch').find('table').attr('rh');
        var trNum = $('#div_complexSearch').find('tr').length;
        var h = trNum * trh + 12 + 10 + btnH;
        if (trNum > 1) {
            h = trNum * trh + 12 + 10 + btnH;
        }
...
function SimpleSearch(obj, value, name) {
    ...
    if (treeDom && treeDom.length > 0) { //存在左侧树
        ...
        if (node && node.id != -1) {
            var oo = {};
            oo[treeField] = node.id != '-1' ? node.id : node.text;
...
function Edit(obj) {
    ...
    if (formUrl == undefined || formUrl == null || formUrl.length == 0) { //通用新增页面
        ...
        if (btnVerifyText == undefined || btnVerifyText == '') {
            var spanTextDom = $(obj).find('span.l-btn-text');
            if (spanTextDom.length > 0 && spanTextDom.text())
                btnVerifyText = spanTextDom.text();
            else
                btnVerifyText = "编辑";
        }
        if (!isDraft) {
            ...
            if (page == "grid" || page == "view") { //主网格页面或主网格下方明细网格或者主从查看页面
                ...
                GridOperateVerify(moduleName, "删除", ids, function (errMsg) {
                    ...
                    if (errMsg != "pass" && errMsg && errMsg.length > 0) {
                        ...
                        if (typeof (AfterDeleteRowAction) === "function") {
                            //删除行之后动作
                            AfterDeleteRowAction(gridId, copyRows);
                        }
                    }
                });
            }
...
function ViewRecord(obj) {
    ...
    if (!titleKeyValue || titleKeyValue.length == 0) {
        var row = null;
        if (selectId) { //从当前按钮中找对应的记录Id来得到选择行
            var rows = gridObj.datagrid("getRows");
            for (var i = 0; i < rows.length; i++) {
                var tempRow = rows[i];
                if (selectId == tempRow["Id"]) {
        ...
        if (!row) { //没有自定义记录行,从选中行中取
            row = GetSelectRow(gridId); //获取选中行
        }
        if (!row) {
            topWin.ShowMsg("提示", "请选择一条记录！", null, null, 1);
            return;
...
function ImportModel(obj) {
    ...
    url += '&r=' + Math.random();
    topWin.OpenDialog('数据导入－' + moduleDisplay, url, toolbar, 600, 320, 'eu-icon-export');
}
...
function ExportModel(obj) {
    var execFun = function () {
        ...
        var toolbar = [{
            ...
            handler: function (e) {
                ...
                iframe[0].contentWindow.GetCondition(function (type, conditions) {
                    ...
                    var condition = GetLocalQueryString("condition");
                    if (condition && condition.length > 0) {
                        data['condition'] = condition;
                    }
                    var where = GetLocalQueryString("where");
                    ...
                    if (downHost && downHost.length > 0) {
                        var user = topWin.GetUserInfo();
                        if (user && user.UserName) {
                            data.un = user.UserName;
                            url = downHost + '/api/DataApi/ExportModelData.html';
                        }
                    }
                    //开始导出数据
                    topWin.DisableTopDialogBtn('btnOk'); //禁用确定按钮
                    ...
                    topWin.CloseDialog();
                    topWin.ShowMsg('导出提示', '正在导出，可能需要等待较长时间，等待期间您可以处理其他工作，请不要重复点击！', null, 'center', 5);
                });
            }
        }, {
            id: 'btnClose',
        ...
        var url = '/Page/ExportModel.html?moduleId=' + moduleId + '&cc=' + count + '&viewId=' + viewId;
        if (isNfm) {
            url += '&nfm=1';
        }
        url += '&r=' + Math.random();
        topWin.OpenDialog('数据导出－' + moduleDisplay, url, toolbar, 650, 360, 'eu-icon-export');
    };
    var user = topWin.GetUserInfo();
    if (user.UserName != 'admin') {
        SnsValid(function (div) {
            execFun();
        }, $(obj).attr("moduleName"));
    }
    else {
    ...
    var ExecuteStartFlow = function () {
        ...
        topWin.ShowConfirmMsg(msgTitle, msg, function (action) {
            if (action) {
                ...
                if (typeof (OverGridStartFlowSetFlowName) == 'function') {
                    //获取各个记录对应发起的流程版本的流程名称，以逗号分隔，顺序与rows的顺序保持一致
                    var flowNames = OverGridStartFlowSetFlowName(rows);
                    if (flowNames && flowNames.length > 0) {
                        paramas.flowNames = flowNames;
                    }
                }
...
function AddRow(gridId, moduleId, moduleName, notBindEvent) {
    ...
    if (detailEditMode == 3) { //列表行编辑模式
        ...
        if (isRowExpand) { //为行展开编辑模式，默认展开
            ...
            for (var i = 0; i < detailRows.length; i++) {
                ...
                if (td && td.length > 0) {
                    td.find('span.datagrid-row-collapse').click();
                }
            }
        }
    }
    ...
    if (!notBindEvent) {
        //绑定控件事件
        BindGridControlEditorEvent(gridId, 0, moduleId, moduleName, false);
        var allRows = GetCurrentRows(gridId);
        for (var ii = 0; ii < allRows.length; ii++) {
            ...
            //重新绑定新增行的控件事件
            BindGridControlEditorEvent(gridId, rowIndex, moduleId, moduleName, false);
            ////重新处理行中的ddv及table的id
            //var prefixRid = GetGridRowIdPrefix(gridObj.parent());
            //var ddv = $('#' + prefixRid + rowIndex).next().find('div.ddv');
            //if (ddv && ddv.length > 0) {
            //    var divId = 'div_' + moduleId + rowIndex;
            //    ddv.attr('id', divId);
            //    var tb = ddv.find('table.ddv');
            //    if (tb.length > 0) {
            //        var tbId = tb.attr('id');
            //        tbId = tbId.substr(0, 40) + rowIndex;
            //        tb.attr('id', tbId);
            //    }
            //}
        }
...
function EditRow(gridId, rowIndex, moduleId, moduleName, notBindEvent) {
    ...
    if (!notBindEvent) {
        //绑定控件事件
        BindGridControlEditorEvent(gridId, rowIndex, moduleId, moduleName, true);
    }
}
...
function UpdateRowFieldValue(gridId, rowIndex, field, value) {
    try {
        var gridObj = $("#" + gridId);
        var updateJson = {};
        var td = GetGridEditorTdCell(rowIndex, field, $('#' + gridId).parent());
        if (td.find('input').length > 0) { //编辑网格
            var column = GetGridColumn(gridId, field);
            var editor = GetGridEditorControl2(gridId, rowIndex, field);
...
//row:行对象
//maxW:最大宽度
function DetailGridRowFormatter(gridId, moduleId, moduleName, index, row, maxW) {
    $('#' + gridId).attr('nud', '1');
    var divId = "div_" + moduleId + index;
    var div = "<div id=\"" + divId + "\" class=\"ddv\" style=\"width:" + maxW + "\"></div>";
                ...
                tbObj.datagrid({
                    ...
                    onLoadSuccess: function () {
                        ...
                        if (page == 'add' || page == 'edit') {
                            ...
                            tempTbObj.attr('gdId', gridId);
                            tempTbObj.attr('gdIndex', index);
                            tempTbObj.attr('moduleId', moduleId);
                            tempTbObj.attr('moduleName', moduleName);
                            BindRowDetailControlEvent(tbId, row, gridId, index, moduleInfo);
                        }
                        if (typeof (DetailExpandGridLoadSuccess) == 'function') {
                            DetailExpandGridLoadSuccess(tbId, row, index, gridId);
                        }
                    },
...
//tempIndex:主网格行号
//tempModuleInfo:模块信息
function BindRowDetailControlEvent(tbId, row, tempGridId, tempIndex, tempModuleInfo) {
    var gridId = tempGridId ? tempGridId : $('#' + tbId).attr('gdId');
    var index = tempIndex ? tempIndex : $('#' + tbId).attr('gdIndex');
    var moduleInfo = tempModuleInfo ? tempModuleInfo : { moduleId: $('#' + tbId).attr('moduleId'), moduleName: $('#' + tbId).attr('moduleName') };
    //绑定控件事件
    //获取当前所有列
                ...
                case 'checkbox':
                    {
                        editor.change(function () {
                            ...
                            if (typeof (OverOnRowDetailFieldValueChanged) == "function") {
                                var oldValue = $(this).attr('checked') == 'checked' ? '0' : '1';
                                var tempTbId = $(this).attr('tbId');
                                OverOnRowDetailFieldValueChanged(moduleInfo, tempField, value, oldValue, index, tempTbId);
                            }
                        });
                    }
                    break;
                case 'numberbox':
                    {
                        editor.numberbox({
                            onChange: function (newValue, oldValue) {
                                ...
                                if (typeof (OverOnRowDetailFieldValueChanged) == "function") {
                                    var tempTbId = $(this).attr('tbId');
                                    OverOnRowDetailFieldValueChanged(moduleInfo, tempField, newValue, oldValue, index, tempTbId);
                                }
                            },
                            value: row && row[f] ? row[f] : ''
                        });
                    }
                    break;
                case 'combobox':
                    {
                        ...
                        editor.combobox({
                            ...
                            onSelect: function (record) {
                                if (typeof (OverOnRowDetailFieldSelect) == "function") {
                                    var tempField = $(this).attr('fieldName');
                                    var tempTbId = $(this).attr('tbId');
                                    OverOnRowDetailFieldSelect(record, tempField, valueField, textField, index, tempTbId);
                                }
                            },
                            value: row && row[f] ? row[f] : ''
                        });
...
function OnSelect(rowIndex, rowData, gridId, moduleId, moduleName) {
    if (page == "grid" && gridId == "mainGrid") {
        ...
        LoadFlowChart(moduleId, rowData['Id'], rowData['TaskToDoId'], rowData, gridId);
    }
...
function OnLoadSuccess(data, gridId, moduleId, moduleName) {
            ...
            if (page == 'edit') { //主从编辑页面
                    ...
                    if (isRowExpand) { //为行展开编辑模式，默认展开
                        setTimeout(function () {
                            for (var i = 0; i < detailRows.length; i++) {
                                ...
                                var td = GetGridSysEditorTdCell(rowIndex, '_expander', gridObj.parent());
                                if (td && td.length > 0) {
                                    td.find('span.datagrid-row-expand').click();
                                }
                            }
                        }, 100);
                    }
                ...
                if ($("a[id^='flowBtn_']").length > 0 && $("a[id^='flowBtn_']").eq(0).attr('parentToDoId')) {
                    if (data && data.rows && data.rows.length > 0) {
                        ...
                        LoadFlowChart(moduleId, recordId, todoId, tempRow, gridId);
                    }
                }
            }
    ...
    if (page == 'grid' && gridId == 'mainGrid') {
        ...
        if ($("div[id^='region_south']").length > 0 && $("div[id^='region_south']").css('display') != 'none') {
            ...
            if (data && data.rows && data.rows.length > 0) {
                setTimeout(function () {
                    var newh = 36 + data.rows.length * 32 + 30 + 33;
                    var groupTitle = $('div.datagrid-group');
                    if (groupTitle.length > 0)
                        newh += 26 * groupTitle.length;
                    if ($('#div_complexSearch').attr('mutistyle') == '1') {
                        var temph = parseInt($("div[id^='grid_toolbar_']").eq(0).height());
                        if (temph == 0) {
                            var trh = $('#div_complexSearch').find('table').attr('rh');
                            temph = $('#div_complexSearch').find('tr').length * trh + 28 + 5;
                        }
                        newh += temph;
                    }
...
function GridTreeNodeClick(node) {
    if ($('#div_complexSearch').attr('mutistyle') == '1') { //当前为复杂搜索
