...
function FormatOp(value, row, index) {
    var btn = "<i title='增加条件' class='iconspirt eu-p2-icon-add_other' onclick='AddRow()' style='width:18px;cursor:pointer'></i>";
    btn += "<i title='移除条件' class='iconspirt eu-icon-close' onclick='RemoveRow()' style='margin-left:8px;width:18px;cursor:pointer'></i>";
    return btn;
}
...
function OnFieldSelected(record) {
    if (fieldInfos != null && fieldInfos.length > 0) {
        ...
        if (fieldInfo != null) {
            ...
            valueTdDom.html('<input style="width:97%;height:28px;" id="value_' + rowIndex + '" type="text" />');
            ...
            else if (fieldInfo.ForeignModule && (fieldInfo.ControlType == 0 || fieldInfo.ControlType == 8 || fieldInfo.ControlType == 17)) {
                for (var j = 0; j < methodData.length; j++) {
                    if (methodData[j].Id != 1 && methodData[j].Id != 2 &&
                        methodData[j].Id != 3 && methodData[j].Id != 4) {
                        tempMethodData.push(methodData[j]);
