            ...
            $foreinInputs.each(function () {
                ...
                if (foreinName) {
                    ...
                    if (obj.length > 0) {
                        var v = $(this).attr('v');
                        if (v == undefined) v = '';
                        var tempDom = $(this).next('span').find('input.textbox-text');
                        if (v.length == 0 && $(this).attr('isRequired') == '1' && tempDom.attr('disabled') != 'disabled') {
        ...
        if (!detail) { //正常编辑保存
            ...
            if (nosave != "1") { //需要保存数据
                ...
                if (typeof (OverMainModuleDataHandleBeforeSaved) == "function") {
                    OverMainModuleDataHandleBeforeSaved(data, updateFields);
                }
                if (!isNoFastSave) { //快速保存方式
                ...
                if (hasDetail) { //有明细模块
                    ...
                    if (details.length > 0) { //有明细数据
                        if (typeof (FunBeforeGetDetailDataOveride) == "function") {//重新明细要保存的数据，可进行筛选和额外附加
                            details = FunBeforeGetDetailDataOveride(details);
                        }
            ...
            if (btnTagId != null && btnTagId.length > 0 && btnTagId.indexOf('flowBtn_') > -1) {
                ...
                if (toDoTaskId != undefined && toDoTaskId != null && toDoTaskId.length > 0) {
                    ...
                    msgTitle = '发起流程提示';
                    successMsg = '流程发起成功！';
                    var startFlowName = $(obj).attr('startFlowName'); //指定了流程版本的流程名称
                    if (startFlowName) {
                        formObject.StartFlow = startFlowName;
                    }
                }
                ...
                if (returnNodeId != undefined && returnNodeId != null && returnNodeId.length > 0) {
                    ...
                    formObject.IsReturnBack = $(obj).attr('isReturnBack') == '1';
                }
            ...
            if (editMode == 2) { //弹出框编辑模式
                ...
                if (typeof (OverDialogDetailEditBeforeSave) == "function") {
                    OverDialogDetailEditBeforeSave(row, gridId, page, winObj);
                }
                if (page == "add") {
...
function SubmitFlow(obj) {
    ...
    if (!msg)
        msg = '确定要提交流程吗？';
    topWin.ShowConfirmMsg('流程提交', msg, function (action) {
        if (action) {
            Save(obj);
        }
    });
...
function ApprovalFlow(obj, confirmBackFun) {
    ...
    var msg = '确定' + btnText + '吗？';
    var backFun = null;
    if (typeof (BeforeApproval) == "function") {
        BeforeApproval(obj);
    }
...
function BackFlow(obj) {
    ...
    ExecuteCommonAjax('/' + CommonController.Async_Bpm_Controller + '/LoadBackNode.html', { toDoTaskId: tempTodoId }, function (result) {
        if (result != null && result.html != null) {
            topWin.OpenOkCancelDialog('选择回退结点', result.html, 400, 240, function (iframe, backFun) {
                var returnNode = topWin.$("input[name='backNodes']").val();
                var isReturnBack = topWin.$('#isReturnBack').length > 0 && topWin.$('#isReturnBack').attr('checked') == 'checked' ? '1' : '';
                $(obj).attr('returnNodeId', returnNode);
                $(obj).attr('isReturnBack', isReturnBack); //退回节点审批后是否直接返回当前节点
    ...
    $.each(data, function (index) {
        ...
        if (handleForeignField) {
            if (field != null && field.ForeignModuleName.length > 0 && field.ControlType != 100) { //是外键字段，并且不是标签字段
                var textName = name.substr(name.length - 2) == "Id" ? name.substr(0, name.length - 2) + "Name" : name + "Name";
                var textValue = "";
                try {
                    textValue = $("#" + name, form).textbox("getText");

                } catch (e) {
                    try {
                        textValue = $("#" + name, form).val();
                    } catch (e) {
                        textValue = $("#" + name, form).html();
                    }
                }
                formData[textName] = textValue;
                try {
                    //判断v与value不一致
                    var v = $("#" + name, form).attr("v");
                    if (v != value && v != undefined && v != null && v != "") {
                        formData[name] = v;
                    }
                }
                catch (e) {
...
//oldValue:改变前的值
//obj:当前控件
function OnFieldValueChanged(moduleInfo, fieldName, newValue, oldValue, obj) {
    ...
    if (typeof (OverOnFieldValueChanged) == "function") {
        OverOnFieldValueChanged(moduleInfo, fieldName, newValue, oldValue, obj);
    }
}
