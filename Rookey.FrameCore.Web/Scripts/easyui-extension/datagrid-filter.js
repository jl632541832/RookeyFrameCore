(function($){
	...
	var extendedOptions = {
		filterMenuIconCls: 'eu-icon-ok',
		filterBtnIconCls: 'eu-icon-filter', 
	...
	function addFilterRule(target, param){
		...
		if (param.op == 'nofilter'){
		    removeFilterRule(target, null); //20170112 dingxin.wang removeFilterRule(target, param.field);
		} else {
		...
		function createFilterButton(container, operators){
			...
			menu.menu({
				...
				onClick:function(item){
					...
					if ((value == null || value == '') && (item.name == 'contains' || item.name == 'beginwith' || item.name == 'endwith')) {
					    return;
					}
