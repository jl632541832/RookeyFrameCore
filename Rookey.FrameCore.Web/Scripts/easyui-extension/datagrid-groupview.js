	...
	onBeforeRender: function(target, rows){
		...
		function getGroup(value) {
		    if (checkDate(value)) { //日期
		        ...
		        //本季度起始日期
		        var quarterStartDate = new Date(y, getQuarterStartMonth(), 1);
		        var groupName = "本年";
		        if (date.getFullYear() == y) { //本年内
		            if (date >= quarterStartDate) { //本季度
		                if (date >= monthStartDate) { //本月
		                    if (date >= weekStartDate) { //本周
		                        groupName = "本周";
		                    }
		                    else {
		                        groupName = "本月";
		        ...
		        if (tempGroup == null) {
		            tempGroup = { value: groupName, rows: [] };
